# apitest — CEL expressions reference
...
ApiTool embeds the Common Expression Language (CEL) for two purposes:
...
|---|---|---|
| `ERR_CEL_PARSE` | Expression syntactically invalid or uses a disabled function | Run `apitest validate` — message names field path and shows up to 200 chars of source |
| `ERR_CEL_TYPE` | Expression compiles but returns a non-bool type | Run `apitest validate` — message names the actual type |

Both error codes surface at `apitest validate` time, not at run time, when the

---
name: apitest
description: Run ApiTool collections and interpret results. Use when the user asks to run, test, hit, exercise, or check an HTTP API; or when the user mentions a collection file (e.g. "run the users collection", "hit the staging API", "test the auth flow"). The CLI never calls an LLM — the agent reads deterministic artifacts (markdown reports + NDJSON event stream) and narrates them.
---

<!-- apitest-skill: claude v1.0 (apitest {{apitest_version}}) -->

# apitest skill

You are driving the `apitest` CLI on behalf of a developer. ApiTool is a
...
- Direct mentions: "use apitest to ...", "apitest run ..."

Before invoking, look at the project layout. ApiTool projects have an
`apitest.yaml` at the root, a `collections/` directory with one or more
...
The `apitest.yaml` `output:` block declares the output format and artifact
paths — that is why the agent does not need to pass `--format markdown
--report responses/ --events .apitest/run.ndjson`. If the user names an
...
For a one-shot URL with no collection, use `apitest exec`:
...
After every `apitest run`, three artifact streams exist:
...
- `responses/<slug>.md` — one file per main-phase request. Slug is derived
  ...
  delimited by `<!-- BEGIN apitest:response id=<request_id> slug=<slug>
  run=<run_id> --> ... <!-- END ... -->` sentinels. Re-runs splice only the
  bytes between the sentinels; agent or human notes outside the block
  survive verbatim.
- `.apitest/run.ndjson` — the structured event stream. One JSON object per
...
- `run_id` (32-char lowercase hex) — one per `apitest run` invocation.
...
context, then at `.apitest/run.ndjson` for structured error details. If the
...
`run.error` event in `.apitest/run.ndjson`.
...
| 2 | Guard rail tripped (request limit exceeded) | `.apitest/run.ndjson` (`run.end` event has `exit_code: 2`; skipped requests show `outcome: skipped`) | The collection ran past the safety cap. Suggest splitting into smaller collections. |
...
| 4 | Non-assertion runtime error | `.apitest/run.ndjson` (`request.end` event with `outcome: error`, `error.category: network`) | Connection refused / DNS failure / TLS error. Suggest checking the URL and environment variables. |
| 5 | Undefined or circular variable | `.apitest/run.ndjson` (`run.error` event names the variable) | Tell the user which variable is missing and the most likely producer (env file, `--var`, an upstream extract). |
...
| 9 | License grace period expired | stderr (`feature_gated: grace period expired ...`) | Tell the user to run `apitest license --validate` and re-authenticate. |
...
- Use `--only "<name>"` to iterate fast on a single request. Setup and
  ...
  `apitest watch` for tight inner-loop iteration.
- If a request needs a new variable, prefer setting it in
  `environments/<env>.yaml` (commit-safe) over inline literals.

## Notes

- This skill was scaffolded for apitest **{{apitest_version}}**. If your
  CLI binary is much newer or older, regenerate it with
  `apitest init --skill claude` in a fresh directory and diff.
- For a human-friendly walk-through of the same workflow, see
  `docs/MANUAL.md` §4.9 ("Driving apitest with Claude Code").
- The skill is checked in. Edit it freely for your team's conventions —
  the next `apitest init --skill claude` will not overwrite a
  pre-existing `.claude/skills/apitest/SKILL.md`.

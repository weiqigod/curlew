# apitest — Output formats reference
...
Declare output in `apitest.yaml` so every `apitest run` uses the same format
without flags:
...
`apitest init --skill claude` scaffolds this block automatically.
...
- **`responses/<slug>.md`** — per-request detail: status, headers, body,
  ...
  block delimited by `<!-- BEGIN apitest:response ... -->` sentinels so
  re-runs splice only that block.

## NDJSON event stream

Enable with `events: .apitest/run.ndjson`. One JSON object per line:

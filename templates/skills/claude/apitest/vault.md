# apitest — Vault and secret providers reference
...
ApiTool integrates with secret managers so credentials never appear in
collection YAML. Configure providers in `apitest.yaml`:
...
ApiTool resolves the vault reference at run time and substitutes the secret
...
ApiTool calls the command with the vault path as the first argument and expects
...
ApiTool redacts sensitive values in:
...
- Vault lookups add latency. ApiTool caches resolved secrets for the lifetime

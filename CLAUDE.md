...
- **docs/TESTAPI_SPECIFICATION.md** — Mudflat, the dedicated test API for dogfooding the CLI against a server curlew did not write. **Phases 1, 2 and 3 are implemented in `testapi/`** and run as `ci-local.sh` gate steps (dogfood suite, --parallel rendezvous, and the gaps/redaction/openapi/crosscheck harnesses). Dogfooding has found **sixteen defects — all fixed** as of 2026-08-13: five from Phases 1–2 (§11A, §11B), ten from Phase 3 (§11C.1–10), and an eleventh Phase-3 one found while fixing them (§11C.11). Every reproduction was *moved*, not deleted — into a passing collection, into `expected-failures.yaml` inverted so it still fails but for the right reason, or into a unit test — so a closed gap keeps the test that proves it stayed closed. Two harnesses were retired by their own promotion instructions (`timing.sh`, and the `*.run-abort.yaml` category in `gaps.sh`); `openapi.sh` was inverted from "these must be rejected" to "these must import". `testapi/harness/redaction-known-leaks.txt` is empty, which means every output surface is enforced rather than baselined. §9.N (the nine-port TLS matrix) is deliberately reduced to one listener — §14.3 has the measurement showing no client-side route to trusting mudflat's CA exists on this toolchain

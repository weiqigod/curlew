...
`docs/MANUAL.md` is the authoritative feature reference; `docs/SPECIFICATION.md` describes the platform as designed and no longer matches the CLI. The Go CLI builds (`go build ./cmd/curlew`) and runs; the .NET backend, web dashboard, and local web UI are all built. The five convergence Playwright E2E specs seed the backend over HTTP (no `curlew` binary involved) and run again in `scripts/ci-local.sh`; the remaining `web/tests/e2e/org-*` specs mock backend routes, are not part of any gate, and currently fail against the docker stack. New backlog work resumes the task lifecycle below.

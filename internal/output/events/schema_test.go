...
// compileEventSchema compiles the current (v1.6) JSON Schema. Callers that
...
func compileEventSchema(t *testing.T) *jsonschema.Schema {
	t.Helper()
	return compileEventSchemaVersion(t, "v1.6")
}
...
// buffer and validates every line against the v1.6 JSON Schema.
...
// testdata/golden/ and validates each line against the v1.6 JSON Schema.
...
// docs/events-schema/v1.6.json, and that every field without an ",omitempty"
...
// ```json or ```ndjson from docs/EVENTS_SCHEMA_v1.6.md and validates each
// non-blank line against the v1.6 JSON Schema.
//
...
func TestSchema_MarkdownExamplesValidate(t *testing.T) {
	...
	docPath := eventSchemaDocPath(t, "v1.6")
...
// TestSchema_v16_validates compiles docs/events-schema/v1.6.json and validates
...
// current schema.
func TestSchema_v16_validates(t *testing.T) {
	sch := compileEventSchemaVersion(t, "v1.6")

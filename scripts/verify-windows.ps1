...
try {
    ...
    Invoke-Checked $go "test" "./..." "-count=1" "-timeout=15m"
    $env:CC = $cCompiler
    $env:CGO_ENABLED = "1"
    Invoke-Checked $go "test" "-race" "./..." "-count=1" "-timeout=20m"
    Invoke-Checked $go "test" "-coverprofile=coverage-windows.out" "./..." "-timeout=15m"

...
> the only breaking change is `schema_version` changes from `"0.1"` to `"1.0"`.
...
rules, and the v0.x stability policy. You do not need to read the source code to
...
- **Schema version** — every event carries `"schema_version": "0.1"`. Consumers
...
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"0.1"` |
...
```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"check indentation on line 5","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example:

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"test","source_file":"tests.yaml","source_line":12}
```
...
| error | object | no | Error payload when `outcome=error`; same structure as `run.error.error` |

Minimal example (`outcome=passed`):

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with status code and bodies:

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with network error (`outcome=error`):

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_TIMEOUT","message":"request timed out after 5000ms","hint":"increase the timeout or check the host is reachable"}}
```

Example with truncated response body:

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"<first 2048 bytes of body>","response_body_size":32768,"response_body_truncated":true}
```
...
```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```
...
| `input` | Invalid CLI argument or missing required flag | Re-read help text; check invocation |
| `internal` | Unexpected internal error (always a bug) | Report with the full event stream |

### Error codes by category

Error codes are stable, SCREAMING_SNAKE_CASE identifiers. They are additive in
v0.x (new codes may be added without a version bump). The following table lists
representative codes per category. For the full set, see `internal/errors/` and
the `hints_init.go` files in each package.
...
| `PARSE_CIRCULAR_INCLUDE` | A collection's include: chain is circular | "Break the include: cycle. A collection may not transitively include itself." | `internal/parser` |
| `PARSE_CIRCULAR_FILE_REFERENCE` | A request_file: chain is circular | "Break the cycle: a request_file: must not transitively reference itself." | `internal/parser` |

**network** — produced by `internal/errors` (via `ClassifyNetworkError`), `internal/httpexec`
...
**config** — produced by `internal/config`, `internal/variable`
...
```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"check indentation","file":"tests.yaml","line":5}}
```

```json
{"schema_version":"0.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_DNS","message":"no such host: api.example.com","hint":"check your DNS configuration"}}
```
...
### Additive changes (allowed in v0.x without a version bump)

The following changes may be made to the stream format in any v0.x release. A
...
v1.0 may be cut only after the M6-007 validation harness passes against every
canonical broken fixture:
...
Until v1.0 is promoted, treat the stream format as unstable. The `schema_version`
field will transition from `"0.1"` to `"1.0"` atomically; a consumer MUST check
this field on every event.
...
3. **Tolerate unknown optional fields.** The stability policy allows additive
   optional fields in v0.x. A conforming consumer must not error on unknown
   field names.

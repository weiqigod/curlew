...
*A tutorial and reference for the local CLI, browser UI, and coding-agent workflow.*

Start with [installation](../README.md#install), the [agent guide](AGENT_GUIDE.md), or the [local UI walkthrough](#16-local-browser-ui). Examples using `example.com`, provider credentials, or application-specific endpoints are adaptation recipes: supply the named files, variables and API before running them. For complete examples with a local server, use the [cookbook](../site/README.md).
...
- [1.6 Local browser UI](#16-local-browser-ui)
...
It is the tool you reach for when Postman's clickable UI starts fighting you: when you want diffs instead of workspaces, pull requests instead of shared accounts, text editors instead of drag-and-drop, and the same tests running in your terminal and in CI without re-exporting anything. It also runs well as a tool invoked by AI coding agents — `run`, `exec`, `validate`, and `info` support `--format json`; `exec` additionally accepts `--non-interactive` and JSON request input from stdin. See [the agent guide](AGENT_GUIDE.md) for the command-by-command contract. Curlew has no MCP server.
...
**Placeholders.** Beginner examples use `https://httpbin.org`, which is a real public HTTP echo service — those examples require internet access and the public service to be available. Local verification uses the cookbook fixtures. Business-logic examples use `https://api.example.com` as a stand-in for your own API.
...
Curlew is a single executable with an embedded browser UI. Release archives for
macOS, Linux and Windows are available through authenticated GitHub access; the
repository is private. See [README installation](../README.md#install) for the
executable download commands and platform selection. Runtime use needs no account.

To build the complete app from a checkout, install Go 1.24+ and Node.js 22+, then:
...
Copy the executable to a directory on your PATH. Untagged source builds report
`0.1.0-dev`; release archives report their release version. Plain `go install`
is supported for CLI-only use, but does not run the frontend build. Use a release
archive or the two-step build above when you want the browser UI.
...
### 1.6 Local browser UI

From the directory containing `curlew.yaml`, run:

```bash
curlew ui
```

Keep that terminal open. Curlew opens your browser on a loopback address, normally
port 8765. If browser launch fails, open the complete URL printed in the terminal.
The URL contains a session token; use the newly printed URL after a restart.

1. Choose the environment in the header and select a collection in the sidebar.
2. Choose **Run all**, or select requests and run the selection. Setup and teardown
   still run when you select a main request.
3. Select a result to inspect its body, headers, assertions, timing and request.
   Failed assertions show expected and actual values. Transport errors have error details.
4. Open the collection in your editor to change YAML; the UI does not edit files.
5. Use History to inspect or compare retained UI runs. Plain CLI runs do not
   automatically appear in UI history.
6. Press `?` for keyboard help. Stop the server with Ctrl+C in its terminal.

Use `curlew ui --no-open --port 8766` to choose a port and open the URL yourself.
An explicitly requested busy port fails with a diagnostic. History is stored under
`.curlew/ui/`; disable it with `ui.history.enabled: false` in `curlew.yaml`.
If the page reports missing UI assets, install the release binary or rebuild the
frontend before rebuilding the Go executable (§1.1).
...
`request_slug`) link each file back to its event stream; see §4.5a for
the data-driven iteration mapping.
...
output. Other outputs can expose sensitive values when that flag is set.
Redaction covers configured sensitive values and recognised fields; review
artifacts before sharing because arbitrary response content may contain secrets.

**Determinism.** Rendering uses stable ordering, but live runs still differ in
run IDs, timings, timestamps, headers and response content. The metadata
section groups volatile fields to make the request and assertion sections
easier to compare.
...
For JSON, `-v` also includes request and response headers; `-vv` adds bodies.
Agents should tolerate these optional fields and use the summary and assertion
results for outcome decisions. Other formats have their own report layouts.
...
For one-shot requests, `exec --log` appends one JSON object per invocation. For
collection runs, use `run --events` instead; `run` does not accept `--log`.

```bash
curlew exec https://api.example.com/users --log runs.jsonl --format json
```
...
`request_id` (`"req-1"` for the single-request exec command) identify the log entry.
They are not IDs from a separate collection run. `exec` does not write the
collection event stream or Markdown reports; those belong to `run`.
...
`schema_version: "1.6"` and `run_id`. Per-request events also carry
`request_id` (usable with `run --only`) and `request_slug` (the expanded request
name, normalised for URLs). Single-request slugs name Markdown files;
data-driven groups have a separate mapping below. The `run.start` event carries an
optional `selection` field listing the `--only` values for the run.

A request.end line looks like:

```json
{"schema_version":"1.6","id":7,"at_ms":250,"kind":"request.end","run_id":"0123456789abcdef0123456789abcdef","request_id":"req-3","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":234,"wave_index":1}
```
...
`--events` and the markdown sentinel block; `request_id` and `request_slug`
link a specific event line to a specific markdown file.
...
uses a `-index` suffix on the first request ID. Match on run ID and this
normalised request ID: event slugs include the expanded row name (`each-1-2`),
while Markdown uses the group slug (`each/iter-0.md`). `exec --log` records
separate one-shot executions and cannot be joined to a collection run.
...
These are CI configuration templates for a project with the named collections and
API credentials. The Curlew repository is private: create a read-only
`REPO_READ_TOKEN` secret for the GitHub example; for GitLab, configure GitHub
Git credentials in the runner before `go install`. `GOPRIVATE` alone does not grant
access. The source install is sufficient for CLI-only CI runs.
...
**`exec --non-interactive`** — accepted by the one-shot command. Do not pass it to `run`, `validate`, or `info`; their normal operation needs no prompt.

**`--dry-run`** — for `run`, print dependency waves without sending requests,
with or without `--show-dependencies`. This checks the plan, not runtime variable
values or API assertions, and prints text even with `--format json`. For `exec`,
resolve and preview one request; command-backed or vault-backed variables may
still execute their configured resolver.

**`--format json`** — available on `run`, `exec`, `validate`, and `info`. `schema` already writes JSON. See the [agent guide](AGENT_GUIDE.md) for output paths, error handling, and the other commands.

A typical agent pattern:

Run this from an existing project containing `collections/users.yaml`, with its
API and environment configured. The preview prints a dependency plan; it does not
produce a JSON results object or resolve values extracted by future requests.
...
curlew run collections/users.yaml --format json \
  --dry-run --show-dependencies                     # 3. preview
curlew run collections/users.yaml --format json     # 4. execute
```
...
Supply secrets through variables and keep output redaction enabled. Dynamic auth
profiles also maintain an obfuscated disk cache (§6.5); inspect artifacts before
sharing. Four ways to supply secrets are:
...
`--allow-sensitive` disables the normal redaction pass for supported run outputs;
Markdown remains redacted. Use it only for deliberate local debugging. Unmarked
or unrecognised sensitive response content still requires review.
...
| `curlew run --report-upload --org …` | `curlew run --format json > results.json`, then `curlew pr-check --results <file>` (§8.1) |
...
The `perf` command turns a single request file into a load test. Its YAML shape
is `{name, request: {method, url, headers, query, body}}`, not a collection with
`requests:`. It uses literal values and does not accept `run` variable flags.
Prepare `requests/hot-path.yaml` for the server you intend to test before running:
...
| `--output` | Output destination: `stdout`, a `.json` file, or an `.html` file |
...
**1. `undefined variable: base_url` (exit 5).** The variable is referenced but never defined. Check `curlew.yaml`, your environment file, `.env`, and the CLI flags. Use `validate` for structural errors and inspect the configured variable sources.
A `run --dry-run` plan does not resolve every runtime variable.

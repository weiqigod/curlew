...
**Part 0 — Preface**
- [P.1 What ApiTool is](#p1-what-apitool-is)
...
- [1.1 Installing apitest](#11-installing-apitest)
...
- [3.6 Project-wide config (`apitest.yaml`)](#36-project-wide-config-apitestyaml)
...
- [4.9 Driving apitest with Claude Code](#49-driving-apitest-with-claude-code)
...
ApiTool is a file-based API testing tool. You write test collections as YAML files, commit them to version control alongside your application code, and execute them with a single static binary called `apitest`.
...
You do **not** need to know Go or the ApiTool source code to work through this manual.
...
**Command style.** Commands are shown as if `apitest` is on your `$PATH`:

```bash
apitest run collections/sample.yaml
```

If you installed to a non-PATH location and are running out of a build directory, mentally substitute `./apitest`.
...
**Variable casing.** Variable names are `snake_case` throughout (`base_url`, `admin_token`, `user_id`) — this matches the scaffold generated by `apitest init`.
...
### 1.1 Installing apitest

ApiTool is a single static Go binary. Two install paths:
...
apitest --help
# apitest — a file-based API testing tool
# ...
```

If `apitest --help` prints the usage summary with the full command list (`run`, `exec`, `validate`, `init`, `info`, `schema`, `watch`, `vault`, `import`, `pr-check`, `worker`, `perf`, `plugins`), you are ready.
...
`apitest init` scaffolds a project. Inspect what it created:

```
demo-api/
├── apitest.yaml              # project config
...
That is end-to-end ApiTool. One command, one file, real HTTP request, assertion checked, exit code 0.
...
The `{{base_url}}` reference resolves to `https://httpbin.org` because that value is defined in `apitest.yaml`'s `variables:` block. ApiTool finds `apitest.yaml` by walking up from the collection file's directory — you can keep your tests in any subdirectory of the project and the config will be picked up automatically.
...
ApiTool publishes its collection JSON Schema at a stable in-repo path so VS Code (with the `redhat.vscode-yaml` extension) can give you autocomplete, hover documentation, and inline validation while you edit collection YAML files.
...
If you are using ApiTool from outside its source tree and don't have the schema files on disk, point `yaml.schemas` at the published URLs instead:

```json
{
  "yaml.schemas": {
    "https://raw.githubusercontent.com/peterlindqvist/apitest/main/schemas/collection-v1.json": "collections/*.yaml",
    "https://raw.githubusercontent.com/peterlindqvist/apitest/main/schemas/project-v1.json":    "apitest.yaml"
  }
}
```
...
**All seven HTTP verbs are supported:** `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`. Method names are case-insensitive in YAML; ApiTool normalizes to uppercase internally.
...
- `headers` is a map. Values are always strings; ApiTool sets the `Content-Type` to `application/json` automatically when `body` is a YAML map.
- `query` is a map, appended to the URL as `?source=apitest&debug=true`. Order is not guaranteed.
...
When `body` is a string, ApiTool sends it as-is — it does not re-encode it as JSON.
...
```bash
apitest run collections/import.yaml --var batch_id=B-2026-04-21
```

ApiTool auto-detects `Content-Type: application/json` from the `.json` extension. An explicit `headers.Content-Type` always wins:
...
For unknown extensions (e.g. `.tmpl`, `.dat`) ApiTool leaves `Content-Type` unset — set it yourself if the server requires it.
...
- Max file size is **50 MB**. Larger files fail at parse time with a structured error — if you legitimately need larger uploads, ApiTool is the wrong tool.
- Missing files fail at parse time, so `apitest validate` catches them before any HTTP traffic.
- Watch mode (`apitest watch`) reruns automatically when either the collection *or* the body file changes.
...
ApiTool body assertions target response fields via JSONPath. If you already know JSONPath, skip ahead. If you don't, this section is all you need.
...
Extracted values are available to every request that runs after the extraction. They are strings (ApiTool converts numbers and booleans via their canonical string form).
...
ApiTool has one interpolation syntax: double curly braces.
...
| 2 | Project variables | `apitest.yaml` → `variables:` |
...
| 6 | Vault secrets | `secrets:` block in `apitest.yaml` |
...
If you set `base_url` in `apitest.yaml` (precedence 2), override it in `environments/staging.yaml` (precedence 3), and then pass `--var base_url=https://local.test` on the command line (precedence 10), the CLI wins.
...
```bash
apitest run collections/users.yaml --env staging --var region=eu-west-1
```
...
`.env` files are for **local** secrets and config that should never be committed. The scaffold from `apitest init` already adds `.env` to `.gitignore`.
...
```bash
apitest run collections/users.yaml \
...
apitest run collections/users.yaml --env-var API_KEY
```
...
apitest run collections/users.yaml --env-var api_token='$CI_API_TOKEN'
```

Quote the value in your shell so the shell doesn't expand `$CI_API_TOKEN` before ApiTool sees it.

### 3.6 Project-wide config (`apitest.yaml`)

`apitest.yaml` lives at the project root. Every command that loads a collection walks up from the collection file's directory looking for `apitest.yaml` or `apitest.yml` and loads it automatically.
...
Example `apitest.yaml` with project-wide output defaults:
...
**`apitest schema --project`** emits the JSON Schema for `apitest.yaml` so you can validate your project config programmatically.
...
- **Project** — `config.locale:` in `apitest.yaml` (project-level default).
...
returned to you. ApiTool does not ship a `$jwtVerify` helper; if you
...
```bash
apitest run collections/users.yaml --seed 42
```
...
ApiTool embeds Google's [Common Expression Language](https://github.com/google/cel-spec) as a strategic escape hatch for predicates and invariants that outgrow the operator catalogue. CEL is parse-checked and type-checked at `apitest validate` time so authoring errors fail before any HTTP request is sent.
...
Both error codes are surfaced by `apitest validate` with the precise field path and a source excerpt (truncated to 200 runes followed by `…`):
...
`apitest validate` performs no HTTP requests; CEL is parse-checked and type-checked offline.
...
```bash
apitest run collections/api.yaml --format json | jq '.summary.failed == 0'
```
...
ApiTool rewrites only the bytes between the sentinels — your notes
...
apitest run collections/users.yaml --format json | jq '.summary.failed'
```
...
```bash
apitest run collections/users.yaml                    # auto: colour if stderr is a TTY
apitest run collections/users.yaml --color=never      # plain text
apitest run collections/users.yaml --color=always     # force on (pipe to less -R)
NO_COLOR=1 apitest run collections/users.yaml        # any non-empty value disables
apitest run collections/users.yaml --no-color        # alias for --color=never
```
...
apitest run collections/users.yaml --only "Get user" --only "Update user"

# Combine with --events to record the selection in the stream
apitest run collections/users.yaml --only "Get user" --events run.ndjson
```
...
**No match → exit 3 before any HTTP.** When no main request matches the provided names, `apitest run` prints the available names to stderr and exits immediately:
...
If apitest cannot analyse the dependency graph (e.g. two setup items extract the
...
**In watch mode.** `apitest watch collections/users.yaml --only "Get user"` works identically — the minimal-setup pruning applies on every file-change trigger.
...
By default, ApiTool aggressively hides values that look sensitive:
...
```bash
apitest run collections/users.yaml --allow-sensitive
```
...
```bash
apitest run collections/users.yaml --log runs.jsonl
```
  ...
  apitest:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version: '1.24'
      - run: go install github.com/peterlindqvist/apitest/cmd/apitest@latest
      - name: Run API tests
        env:
          API_KEY: ${{ secrets.API_KEY }}
        run: |
          apitest run 'collections/**/*.yaml' \
            --env staging \
            --env-var API_KEY \
            --format junit \
...
api-tests:
  ...
  script:
    ...
    - apitest run 'collections/**/*.yaml' --env staging --format junit --report test-results.xml
  artifacts:
    when: always
    reports:
...
All three rely on the same pattern: run ApiTool with `--format junit --report <file>`, then let the CI's built-in JUnit integration handle the reporting.

### 4.7 Project utility commands

**`apitest validate <file|glob>`** — parse and validate collections without executing HTTP:

```bash
apitest validate collections/users.yaml
# collections/users.yaml: OK

apitest validate 'collections/**/*.yaml'
# collections/users.yaml: OK
...
apitest info --format json
```

Useful in AI-agent scripting — feed the JSON output to the agent for it to orient itself.

**`apitest schema`** — emits the JSON Schema describing the collection YAML format. Pipe it into IDE tools that provide autocompletion and inline validation:

```bash
apitest schema > .apitest-schema.json
# Then point your editor's YAML schema config at it.
```

### 4.8 AI-agent commands

ApiTool was designed for agents as well as humans. The relevant affordances:

**`apitest exec <url>`** — one-shot request, no collection file:

```bash
apitest exec https://api.example.com/health
# HTTP 200 — 45ms
```
...
```bash
apitest info --format json                          # 1. orient
apitest validate 'collections/*.yaml' --format json # 2. lint
apitest run collections/users.yaml --format json \
...
### 4.9 Driving apitest with Claude Code

ApiTool ships a default Claude Code skill that teaches a Claude Code agent
how to invoke the CLI, where artifacts land, and how to narrate results.
The skill is opt-in via `apitest init --skill claude`.
...
apitest init --skill claude
```

The following are created or modified in addition to the standard scaffold:

- `.claude/skills/apitest/SKILL.md` — the agent's playbook (new file)
- `apitest.yaml` — `output:` block defaults to `format: markdown` with
  `events: .apitest/run.ndjson` appended (modified)
- `.gitignore` — gains `.apitest/` (modified)

Open the directory in Claude Code (`claude` CLI or the desktop app). Ask:

> Run the sample collection and tell me what happened.

Claude Code reads `.claude/skills/apitest/SKILL.md`, recognises the trigger
...
No extra flags. The `output:` block in `apitest.yaml` declares everything:
markdown reports land in `responses/`, the event stream lands in
`.apitest/run.ndjson`. After the run, Claude reads `responses/run.md`
first (summary), then drills into `responses/<slug>.md` for any failing
request, then `.apitest/run.ndjson` for structured error details. It
...
one-off curl command. Re-run with `apitest watch collections/sample.yaml
...
`.claude/skills/apitest/SKILL.md` for your team's conventions — preferred
environments, project-specific triggers, narration tone, additional
playbook entries. Subsequent `apitest init --skill claude` runs do not
...
Project-level defaults live in `apitest.yaml` under `defaults:` and inherit into every collection:
...
Networks and rate limiters fail in predictable, retryable ways. ApiTool's retry logic targets those: transient 5xx, 429 throttling, connection resets, timeouts.

**Default behavior.** When you enable retry without further config, ApiTool retries on `[429, 502, 503, 504]` for idempotent methods (GET, HEAD, PUT, DELETE, OPTIONS) up to three attempts with exponential backoff starting at 500ms.
...
**Status ranges** accept the explicit `"min-max"` form (`"500-599"`) and the class shorthand `"Nxx"` (`"5xx"`, case-insensitive). A range that parses as neither is a validation error — `apitest validate` reports it rather than silently skipping it.
...
**Retry-After.** When the server sends a `Retry-After` header (either integer seconds `120` or HTTP-date `Wed, 21 Oct 2024 07:28:00 GMT`), ApiTool honors it — up to a 30-second cap — overriding the computed backoff. Set `respect_retry_after: false` to ignore it.
...
**Idempotency warning.** POST and PATCH are not idempotent. Retrying a POST that succeeded server-side but failed client-side (response dropped) can create duplicates. ApiTool warns when you enable retry for non-idempotent methods; either restrict retries with `methods:`, or put `POST` in `do_not_retry_on.methods`, or ensure your endpoint is idempotent (e.g., via an idempotency key).
...
```bash
apitest run collections/users.yaml --confirm-large-dataset
```
...
For collections where requests are independent of each other, ApiTool can run them concurrently. You declare dependencies; ApiTool computes the execution waves.
...
```bash
apitest run collections/order-flow.yaml --parallel
```

ApiTool computes waves by topological sort:
...
```bash
apitest run collections/order-flow.yaml --show-dependencies
...
```bash
apitest run collections/order-flow.yaml --show-dependencies --dry-run
```
...
```bash
apitest watch collections/users.yaml
```

ApiTool runs the collection once, then re-runs it every time the file changes. Pass `--clear` to clear the terminal between runs:

```bash
apitest watch collections/users.yaml --clear
```
...
```bash
apitest run collections/users.yaml --dry-run
```
...
Testing real APIs means handling real credentials. ApiTool's stance: never store secrets; read them from your environment or an external vault on every run.

### 6.1 Mental model

ApiTool never writes a secret to disk under any user-visible path. It supports four ways to supply secrets, in roughly increasing sophistication:

1. **OS environment** — you set env vars in your shell or CI, ApiTool imports them (§3.5).
...
4. **Vault providers** — ApiTool talks to AWS / Azure / HashiCorp / GCP / 1Password for you (§6.4).
...
apitest run collections/users.yaml --var api_key=sk_test_abc
...
Whichever form you use, mark the variable `!sensitive` or name it with a heuristic-matching prefix (`*_secret`, `*_token`, `*_password`) so ApiTool redacts it in output (§4.4, §6.7).

### 6.3 `from_command` variables

When you want ApiTool to pull a value from any command at run time:
...
For each supported vault, ApiTool expects the provider's CLI to be installed and authenticated. ApiTool does not embed SDKs; it shells out.
...
**The `#field` suffix.** For providers whose secrets are structured (AWS, 1Password, HashiCorp), `path#field` extracts a single field; without `#field`, ApiTool expects the value to be a plain string.
...
For token-based auth flows where the test suite itself needs to log in, ApiTool supports named auth profiles that each run a collection to produce an auth token.

**Define profiles in `apitest.yaml`:**
...
The auth collection runs before any request that declares `auth: admin`. The extracted value is cached for `cache_ttl` seconds in `.apitest/cache/` (obfuscated, never plain text). `refresh_on_failure: true` retries once on failure — useful for token expiry.

### 6.6 Shared vault templates

For teams with shared vault configurations, point ApiTool at a team config file via `APITEST_TEAM_CONFIG`:

```bash
export APITEST_TEAM_CONFIG=~/team/apitest-team-config.yaml
apitest run collections/users.yaml --env staging
```
...
For local testing and CI where you don't want to actually hit a vault, set `APITEST_VAULT_STUB=1` and ApiTool uses an in-memory stub.
...
### 6.9 Logging in to the ApiTool backend

For backend-connected features (`--report-upload`, PR checks, team vault fetch, scheduled runs), `apitest` needs to know which backend it is talking to and who the authenticated user is.
...
Once authenticated, `apitest run --report-upload --org <slug> --pr <n> --repo <owner/repo>` uploads the run, posts a GitHub check-run via the GitHub App installation configured for your org, and prints:
...
ApiTool is not only for JSON REST APIs. It speaks GraphQL and WebSocket as first-class protocols, and can import OpenAPI specs.
...
The default is `fail`. Set a project-wide default under `apitest.yaml`:
...
```bash
apitest import openapi docs/openapi.yaml
...
```bash
apitest import openapi docs/openapi.yaml --output collections/imported.yaml
```
...
- Environment-specific hostnames (the spec's `servers[0].url` is used as `base_url` and you'll likely want to move it to `apitest.yaml`).
...
Upload run results to the ApiTool backend for centralized reporting:
...
```bash
export APITEST_BACKEND_URL=https://api.apitest.example.com
export APITEST_BACKEND_TOKEN=<bearer-token>
```
...
With a PR number and repo, ApiTool posts a commit status check summarizing the run:
...
For workflows where the upload and check posting are separate steps (e.g. orchestrated by a scheduler), use `apitest pr-check` explicitly.

### 8.3 Web dashboard

The backend hosts a web dashboard where teams can browse uploaded runs, diff environments, and track flakiness over time. The dashboard is out of scope for this manual — treat it as the remote equivalent of `apitest info` plus history.
...
```bash
export APITEST_COORDINATOR_URL=https://coordinator.example.com
export APITEST_BACKEND_TOKEN=<bearer>
apitest run 'collections/**/*.yaml' --workers 10 --org acme
```
...
```bash
apitest perf requests/hot-path.yaml \
  --vus 50 \
  --duration 2m \
  --ramp-up 30s \
  --rps 500 \
...
> **Safety reminder.** Load testing generates real HTTP traffic. Before pointing `perf` at any endpoint you do not own, verify that load testing is authorized. ApiTool is a tool, not a policy — the responsibility for where you send requests is yours. For production systems, hitting anything beyond a staging environment without approval is a good way to trigger a billing or capacity incident.

---

## Part 10 — Plugins

Plugins are external processes that extend ApiTool with custom logic at three points in the request lifecycle. They are discovered via the `APITEST_PLUGINS` env var. Built-in signers (the `signing:` field — see §6.8, shipping in aws-sigv4 and oauth1 forms) and built-in dynamic functions (`{{$...}}` — see §3.7) require no plugin binary; the external-process plugin path described below is for custom hooks beyond what the built-ins cover.
...
ApiTool looks up plugins via `APITEST_PLUGINS`:
...
- A directory (scanned for executables named `apitest-*`).
...
```bash
apitest plugins list
# NAME              VERSION  HOOKS
# apitest-myhook    0.3.1    on_request
# apitest-slack     1.0.0    on_result
```
...
Plugins are external processes speaking JSON-RPC 2.0 over stdin/stdout (newline-delimited). At startup, ApiTool sends a handshake:
...
**`on_request`** — apitest sends the request envelope; plugin may return changes:
...
// plugin → apitest — only non-zero fields override
...
- Plugin stderr is forwarded to ApiTool's stderr — your plugin can log without any extra wiring.
...
**`apitest --help`** / **`apitest --version`** — help or version, respectively.

**`apitest exec <url>`** — execute a single request. Flags:
...
**`apitest import openapi <spec>`** — convert an OpenAPI 3.x spec to a collection. `--output <file>` sets the destination.

**`apitest info`** — show project metadata. `--format json` for machine output.

**`apitest init [dir]`** — scaffold a new project (apitest.yaml, .env.example, .gitignore, environments/dev.yaml, collections/sample.yaml).

**`apitest perf <request-file>`** — load test. Flags: `--vus`, `--duration`, `--ramp-up`, `--rps`, `--output`.

**`apitest plugins list`** — list discovered plugins and their hooks.

**`apitest pr-check`** — upload results and post a PR status check.

**`apitest run <file|pattern>`** — execute a collection (or glob-matched collections). Full flag list in §4.1, §5.6, §8.1, §9.1.

**`apitest schema`** — emit the JSON Schema for the collection format to stdout.

**`apitest ui`** — start the local web UI (runner & inspector) on 127.0.0.1. Flags:

| Flag | Meaning |
|---|---|
| `--port <n>` | Listen port (default `ui.port` from apitest.yaml, else 8765; `0` = ephemeral; defaulted busy ports scan +1…+19) |
...
The server binds loopback only and mints a per-start session token (printed in the URL). Files are the only source of truth — the UI never edits them; its single write is the run-history store under `.apitest/ui/` (self-gitignored). Sensitive values are always redacted; there is deliberately no `--allow-sensitive`. Full specification: `docs/UI_SPECIFICATION.md`.

**`apitest validate <file|glob>`** — parse and validate collections without executing. `--format json` for machine output.

**`apitest vault list`** — list configured vault provider profiles.

**`apitest watch <file>`** — run and re-run on file changes. Accepts all `run` flags plus `--clear`.

**`apitest worker --job <id>`** — run as a distributed worker.

### B. Environment variables

| Variable | Used by | Meaning |
|---|---|---|
| `APITEST_BACKEND_URL` | report upload, pr-check | Backend base URL. |
| `APITEST_BACKEND_TOKEN` | report upload, distributed | Bearer token for backend auth. |
| `APITEST_COORDINATOR_URL` | distributed | Coordinator base URL. |
| `APITEST_TEAM_CONFIG` | team vault templates | Path to shared vault config. |
| `APITEST_VAULT_STUB` | team vault templates | `1` enables in-memory stub provider. |
| `APITEST_CONFIG_DIR` | login | Override config directory (`~/.config/apitesttool` by default). |
| `APITEST_PLUGINS` | plugins | Colon/semicolon-separated list of plugin paths. |
...
#### C.4 Project config (`apitest.yaml`)
...
**1. `undefined variable: base_url` (exit 5).** The variable is referenced but never defined. Check `apitest.yaml`, your environment file, `.env`, and the CLI flags. Run with `--dry-run` to surface the error without any HTTP traffic.

**2. `yaml: line X: ...` parse errors (exit 3).** Usually indentation or a missing colon. Run `apitest validate <file>` for a line-pinpointed message.
...
**6. `plugin handshake timeout`.** The plugin executable did not respond to `apitest/hello` in time. Run the plugin binary directly and verify it responds to a hello request on stdin. Check with `apitest plugins list`.
...
If you made it here, you can now do everything ApiTool does:
...
- `apitest schema` — the JSON Schema of the collection format, for IDE autocompletion.

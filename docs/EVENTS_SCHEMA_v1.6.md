# Curlew Agent Event Stream — v1.6
...
- [v1.5 → v1.6 changelog](#v15--v16-changelog)
...
v1.6 makes one additive change to `assertion.result`. A v1.5 consumer is
unaffected.
...
|-------|------|-------------|
| `source_file` | string | The collection or external request file this assertion is written in |
| `source_line` | integer | 1-based line of the assertion within that file |

**Why.** v1.5 let a failing assertion name its request. It still could not say
*where the assertion is written*. On a request carrying a dozen assertions, a
consumer that wanted to open the YAML at the failing line had to search the
file for a JSONPath or a header name and guess which occurrence was the right
one.

`source_line` points at the line a developer would edit to change that
assertion, not at the enclosing request:

| Assertion kind | Line reported |
|---|---|
| `body` | the operator key — `equals:`, `exists:`, `matches:`, … |
| `header` | the operator key |
| `status` | the `status:` value |
| `timing` | the `max_duration_ms:` key |
| `schema` | the `schema:` key |
| `cel` | the `- "<expr>"` list entry |

`source_file` matches the enclosing `request.start`'s `source_file`, including
for requests loaded from an external file — in that case the line is relative
to the external file, which is where the assertion is written.

Both fields are absent for assertions with no YAML origin. Today that is only
the runner-synthesised `graphql_error`.
...
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"1.6"` |
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","curlew_version":"1.0.0","cli_args":["run","tests.yaml"]}
```

Maximal example (all fields, including v1.1 selection):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","curlew_version":"1.0.0","cli_args":["run","tests.yaml","--env","staging","--only","Get user"],"collection_file":"tests.yaml","env_name":"staging","selection":["Get user"]}
```
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example (including v1.2 request_slug):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","request_slug":"get-user-by-id","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"main","source_file":"tests.yaml","source_line":12}
```
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with request_slug and status code:

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with the v1.3 `timing` object (fresh connection, one retry):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"timing":{"dns_us":1234,"connect_us":2100,"tls_us":15400,"ttfb_us":21200,"download_us":900,"total_us":41100,"attempts":2}}
```

Example with a reused connection (no dns/connect/tls phases):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":6,"at_ms":61,"kind":"request.end","request_id":"req-2","outcome":"passed","status_code":200,"duration_ms":4,"timing":{"ttfb_us":3100,"download_us":210,"total_us":3500,"connection_reused":true}}
```
...
| **request_id** | string | yes | Matches the surrounding `request.start` / `request.end`. Positional and run-local — use it to pair within one stream, not to identify a request across runs |
| request_slug | string | no | URL-safe slug of the request, mirroring the paired `request.start`. Stable across runs while the request name is unchanged; also the `responses/<slug>.md` filename. Present for every request parsed from a collection |
| source_file | string | no | Collection or external request file this assertion is written in; matches the enclosing `request.start` |
| source_line | integer | no | 1-based line of the assertion within `source_file` — the operator key for body and header, the `status:` / `max_duration_ms:` / `schema:` key otherwise, the list entry for `cel` |
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","request_slug":"get-user","source_file":"test.yaml","source_line":9,"type":"status","label":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","request_slug":"get-user","source_file":"test.yaml","source_line":9,"type":"status","label":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.6","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```

# ApiTool Agent Event Stream — v1.0
...
- [v0.1 → v1.0 changelog](#v01--v10-changelog)
...
v1.0 is the **first stable release** of the ApiTool agent event stream. It is
promoted from v0.1 after all seven agent-diagnosability harness scenarios pass.

**No breaking changes from v0.1.** The event schema structure, field names, and
semantics are identical. The only observable change is:

- `schema_version` changes from `"0.1"` to `"1.0"` in every emitted event.

**New error codes added in v1.0:**

| Code | Category | Description |
|------|----------|-------------|
| `VAR_UNDEFINED` | `input` | A `{{variable}}` reference could not be resolved. The category changed from `config` to `input` to reflect that this is a caller-supplied value problem, not a collection structure problem. |
| `ASSERTION_FAILED` | `assertion` | A request completed but one or more assertions did not pass. Emitted on `request.end` with `outcome=failed`. |
| `FEATURE_GATE_DENIED` | `auth` | A feature is not available on the current tier. |
| `RUNNER_AUTH_PROFILE_NOT_FOUND` | `auth` | An auth profile referenced in the collection does not exist in the project config. |

**Stability policy changes:**

v0.x allowed additive changes without a version bump. v1.0 promotes all
existing event fields to stable. Removals and renames now require a v2.0 bump.
New optional fields may still be added in v1.x without a bump.
...
- **Schema version** — every event carries `"schema_version": "1.0"`. Consumers
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-21T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml"]}
```

Maximal example (all fields):

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-21T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml","--env","staging"],"collection_file":"tests.yaml","env_name":"staging"}
```
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
| phase | string | no | Run phase (`setup`, `test`, `teardown`) if applicable |
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example:

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"test","source_file":"tests.yaml","source_line":12}
```
...
see [Body truncation and encoding](#body-truncation-and-encoding)).
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with status code and bodies:

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with network error (`outcome=error`):

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_TIMEOUT","message":"request timed out after 5000ms","hint":"increase the timeout or check the host is reachable"}}
```

Example with truncated response body:

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"<first 2048 bytes of body>","response_body_size":32768,"response_body_truncated":true}
```
...
| **type** | string | yes | Assertion type: `status`, `body`, `header`, or `schema` |
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```
...
**input** — produced by `internal/variable`
...
| `FEATURE_GATE_DENIED` | A feature is not available on the current tier | "Upgrade to a tier that includes this feature, or run on a different tier." | `internal/auth` |
...
Use the `kind` and `outcome` fields to route correctly:

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```

```json
{"schema_version":"1.0","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_DNS","message":"no such host: api.example.com","hint":"check your DNS configuration"}}
```

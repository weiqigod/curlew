# ApiTool Agent Event Stream — v1.1
...
- [v1.0 → v1.1 changelog](#v10--v11-changelog)
...
v1.1 is a **backward-compatible, additive release**. No existing fields are
renamed or removed. A v1.0 consumer will continue to work against a v1.1 stream
by ignoring the new optional field.

**New optional field in v1.1:**

- `run.start.selection` (array of string, omitempty) — carries the names passed
  via `--only` for this run. Omitted when `--only` was not supplied. When
  present, each element is the exact case-sensitive main request name the caller
  requested. Whitespace is trimmed by the CLI before the value reaches the event.

**schema_version** changes from `"1.0"` to `"1.1"` in every emitted event.
...
- **Schema version** — every event carries `"schema_version": "1.1"`. Consumers
...
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"1.1"` |
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-24T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml"]}
```

Maximal example (all fields, including v1.1 selection):

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-24T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml","--env","staging","--only","Get user"],"collection_file":"tests.yaml","env_name":"staging","selection":["Get user"]}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example:

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"test","source_file":"tests.yaml","source_line":12}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with status code and bodies:

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with network error (`outcome=error`):

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_TIMEOUT","message":"request timed out after 5000ms","hint":"increase the timeout or check the host is reachable"}}
```

Example with truncated response body:

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"<first 2048 bytes of body>","response_body_size":32768,"response_body_truncated":true}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```
...
```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```

```json
{"schema_version":"1.1","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"error","duration_ms":5001,"error":{"category":"network","code":"NETWORK_DNS","message":"no such host: api.example.com","hint":"check your DNS configuration"}}
```

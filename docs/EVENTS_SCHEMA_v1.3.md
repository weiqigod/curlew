...
- [v1.2 → v1.3 changelog](#v12--v13-changelog)
...
v1.3 is a **backward-compatible, additive release**. No existing fields are
renamed or removed. A v1.2 consumer will continue to work against a v1.3 stream
by ignoring the new optional field.

**New optional field in v1.3:**

- `request.end.timing` (object, omitempty) — connection-phase breakdown for the
  HTTP exchange, measured via `net/http/httptrace`. All duration fields are
  integer **microseconds** (`_us`) — millisecond resolution is useless for
  localhost APIs where DNS/connect take tens of µs; integer values avoid
  float-comparison noise in golden tests; the suffix makes the unit unambiguous.
  Phase fields are omitted when the phase did not occur (e.g. pooled connection:
  no `dns_us`/`connect_us`/`tls_us`). When retries occurred, `timing` reflects
  the **final attempt only** and `attempts` carries the total attempt count.

  | Field | Type | Required | Description |
  |-------|------|----------|-------------|
  | **total_us** | integer | yes | `Do()` entry to body fully read |
  | dns_us | integer | no | DNS lookup duration |
  | connect_us | integer | no | TCP connect duration |
  | tls_us | integer | no | TLS handshake duration |
  | ttfb_us | integer | no | Request written to first response byte |
  | download_us | integer | no | First response byte to body fully read |
  | connection_reused | boolean | no | `true` when a pooled connection was reused |
  | attempts | integer | no | Total attempts; present only when retries occurred (≥ 2) |

  `timing` is absent for skipped requests and for protocols without phase
  capture (WebSocket, GraphQL).

**Documentation corrections in v1.3 (no emission changes):**

- **Phase strings**: the v1.2 document described `phase` as `setup`/`test`/`teardown`,
  but the runner emits `setup`/`main`/`teardown`. The tables and examples below
  match the emitter.
- **`wave_index` quirk**: `request.end.wave_index` is omitted when the value
...
- **`wave_index` schema constraint**: the v1.2 JSON Schema declared
  `minimum: 0`, but sequential streams have always emitted `-1` — every
  sequential `request.end` violated the published v1.2 schema. v1.3 corrects
  the constraint to `minimum: -1`. (Schema fix only; emission unchanged.)

**`schema_version`** changes from `"1.2"` to `"1.3"` in every emitted event.
...
- **Schema version** — every event carries `"schema_version": "1.3"`. Consumers
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"1.3"` |
...
```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example (including v1.2 request_slug):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","request_slug":"get-user-by-id","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"main","source_file":"tests.yaml","source_line":12}
```
...
| timing | object | no | Connection-phase breakdown in integer microseconds; see the [v1.2 → v1.3 changelog](#v12--v13-changelog). Added in v1.3. |

Minimal example (`outcome=passed`):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with request_slug and status code:

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with the v1.3 `timing` object (fresh connection, one retry):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"timing":{"dns_us":1234,"connect_us":2100,"tls_us":15400,"ttfb_us":21200,"download_us":900,"total_us":41100,"attempts":2}}
```

Example with a reused connection (no dns/connect/tls phases):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":6,"at_ms":61,"kind":"request.end","request_id":"req-2","outcome":"passed","status_code":200,"duration_ms":4,"timing":{"ttfb_us":3100,"download_us":210,"total_us":3500,"connection_reused":true}}
```
...
```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.3","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```

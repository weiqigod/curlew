# Curlew Agent Event Stream — v1.5
...
- [v1.4 → v1.5 changelog](#v14--v15-changelog)
...
v1.5 makes one additive change to `assertion.result`. A v1.4 consumer is
unaffected.
...
|-------|------|-------------|
| `request_slug` | string | URL-safe slug of the request this assertion belongs to — the same value the paired `request.start` carries |

**Why.** Through v1.4, `assertion.result` identified its request only by
`request_id`, which the schema defines as an opaque pairing key. It is minted
positionally (`req-1`, `req-2`, ...) in execution order, which makes it correct
for its stated job and unusable for two things consumers were doing with it
anyway:

- **Reading one event.** A failing assertion is the event an agent reads first,
  and it could not name its own request. The consumer had to buffer the stream,
  collect every `request.start`, and join on `request_id` before it could say
  what failed. A consumer tailing a live stream (`curlew watch`) maintains that
  join by hand.
- **Comparing two runs.** The join table is run-local. Insert a request at the
  top of a collection and every id after it shifts, so "the failure at `req-3`"
  refers to a different request in the next run.

`request_slug` is derived from the request name at parse time and is stable
while the name is. It is also the key already used for artifact filenames
(`responses/<slug>.md`), so an agent reading an assertion failure can go
straight to the report for that request.

`request_id` is unchanged and remains the pairing key. Consumers correlating
`request.start` with `request.end` should keep using it.
...
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"1.5"` |
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","curlew_version":"1.0.0","cli_args":["run","tests.yaml"]}
```

Maximal example (all fields, including v1.1 selection):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","curlew_version":"1.0.0","cli_args":["run","tests.yaml","--env","staging","--only","Get user"],"collection_file":"tests.yaml","env_name":"staging","selection":["Get user"]}
```
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example (including v1.2 request_slug):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","request_slug":"get-user-by-id","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"main","source_file":"tests.yaml","source_line":12}
```
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with request_slug and status code:

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```

Example with the v1.3 `timing` object (fresh connection, one retry):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"timing":{"dns_us":1234,"connect_us":2100,"tls_us":15400,"ttfb_us":21200,"download_us":900,"total_us":41100,"attempts":2}}
```

Example with a reused connection (no dns/connect/tls phases):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":6,"at_ms":61,"kind":"request.end","request_id":"req-2","outcome":"passed","status_code":200,"duration_ms":4,"timing":{"ttfb_us":3100,"download_us":210,"total_us":3500,"connection_reused":true}}
```
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","request_slug":"get-user","type":"status","label":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","request_slug":"get-user","type":"status","label":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.5","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```

/* apitest UI — empty project onboarding + YAML validation error panel */
      ...
      <div style={{ width: 560, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--fg2)', marginTop: 4, lineHeight: 1.5 }}>
            apitest reads YAML files from <span className="at-mono">collections/</span> — your files in git are the only
        ...
        <div className="at-mono" style={{
          ...
          {line(<span><span style={{ color: 'var(--fg3)' }}>$ </span><span style={{ color: 'var(--fg0)' }}>apitest init</span></span>)}
          {line('')}
          <div style={{ whiteSpace: 'pre' }}><span style={{ color: 'var(--ok)' }}>  created  </span><span style={{ color: 'var(--fg1)' }}>apitest.yaml</span></div>
          ...
          {line('')}
          {line(<span><span style={{ color: 'var(--fg3)' }}>$ </span><span style={{ color: 'var(--fg0)' }}>apitest run</span><span className="at-cursor" style={{ display: 'inline-block', width: 7, height: 14, background: 'var(--fg2)', marginLeft: 6, verticalAlign: 'middle' }}></span></span>)}
        </div>
    ...
    <div data-screen-label="Validation error" style={{ flex: 1, overflow: 'auto', padding: 'calc(var(--pad) * 2)', minHeight: 0 }}>
      <div style={{ maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--fg3)' }}>
          apitest never edits your files — there is deliberately no fix-it form here.
        </div>

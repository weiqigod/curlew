...
- No CLI subcommand for activation. Existing surface in [cmd/apitest/license.go](cmd/apitest/license.go) is `--validate`, `--refresh`, `--debug`, `export`. A natural extension is `apitest license trial start <feature>` — extend the switch at line 41.
...
7. **Schedule executor architecture (REVIEW.md:191 flagged "needs design pass").** Three options:
   ...
   - **(b) Self-hosted runner.** A `apitest worker --schedule-pull` daemon the customer runs in their VPC; it polls `GET /api/v1/schedules/next-run` (similar to perf workers per REVIEW.md:53), runs the YAML locally, posts results back. **Pros:** no security blast radius, reuses existing distributed-worker code path. **Cons:** customer infra burden — Team-tier customers may not have somewhere to run a daemon.
...
- **No `internal/variable/` integration.** The vault template currently arrives via local YAML file (`APITEST_TEAM_CONFIG`). Spec describes "pull `shared_vault` config from the backend at runtime" but no code does this.
- **Critical: tier gate is registered but not enforced at runtime.** The feature flag at `registry.go:126` is `TierTeam` but `APITEST_TEAM_CONFIG` works at any tier today. **This is the same pattern as the closed M15-001 (plugin loading), M15-002 (SSO), and M15-003 (`from_command`) gaps — registered-but-not-enforced.** M16 must close this enforcement gap as part of the shared-vault slice; it's a small revenue leak today.
...
13. **Propagation cadence.** CLI fetches at the start of every `apitest run` (slow but always-fresh) or caches with a TTL (faster but stale)? Recommendation: TTL of 5 minutes by default with `--refresh-vault` to force; cache file at `~/.config/apitesttool/team_vault.json`. Same shape as the JWKS cache.
...
| D | Schedule executor topology (backend-resident vs self-hosted runner) | Listed | **v3-7** | **Self-hosted runner.** `apitest worker --schedule-pull` polls; backend never sees collection YAML or makes outbound calls toward customer APIs. |
...
| Shared vault for Team tier | 2 | (1) `vault_configs` table + entity + `VaultConfigService` + `VaultConfigTierGate.EnsureTeamOrAboveAsync` + `GET/POST/PATCH/DELETE /api/v1/organizations/{orgId}/vault-config` endpoints + **enforcement at the existing `APITEST_TEAM_CONFIG` load site** (close the M15-style revenue leak); (2) CLI `backend.Client.GetTeamVault()` + cache file at `~/.config/apitesttool/team_vault.json` with 5-min TTL + `--refresh-vault` flag + integration in variable resolver. |

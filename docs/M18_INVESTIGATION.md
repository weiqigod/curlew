...
Sources audited (first verification pass complete; flagged TBDs remain): `docs/REVIEW.md`, `docs/SPECIFICATION.md`, `docs/MANUAL.md`, `docs/M14_INVESTIGATION.md` and `docs/M16_INVESTIGATION.md` (templates), `src/ApiTool.Backend/{Audit,Organizations,Data/Entities,Rbac,GitHub,GitLab,Licensing/Keys,VaultConfig,Notifications}/`, `internal/{output,backend,license,worker}/`, `cmd/apitest/`, `web/src/routes/(app)/org/[slug]/`, `.claude/skills/backlog/milestone-mapping.md`, `management/backlog.yaml`.
...
- "User Data Rights (GDPR/CCPA Compliance)" at `SPECIFICATION.md:6643–6710` covers GDPR via Phase 3 telemetry only: `apitest telemetry delete-request` (30-day window, line 6657), `apitest telemetry export` (immediate, line 6647), 2-year telemetry retention (line 6591).
...
- **No telemetry SDK, no `posthog`/`mixpanel`/`segment`/`amplitude` import, no `apitest telemetry` subcommand** (CLI subcommand registry at `cmd/apitest/main.go:102-155` lists `run, exec, validate, init, info, schema, watch, vault, pr-check, import, login, license, worker, perf, plugins, internal` — no telemetry).
- **No `install_id` storage anywhere.** Files actually present under `~/.config/apitesttool/`: `jwks_cache.json`, `license.json`, `team_vault.json`, `pending-uploads/`.
...
| Telemetry Phase 3 — ingest | 2 | (1) `telemetry_events` table migration + ingest endpoint + 2-year retention + 90-day archive cron; (2) CLI telemetry-emitter (new `internal/telemetry/` package per Q O), opt-in flag + `apitest telemetry {enable, disable, status, export, delete-request}` subcommand + persistent UUID at `~/.config/apitesttool/install_id` (per Q K recommendation) + spec revision to match. |
...
| v4-8 | Telemetry identity model | **Revise spec from per-execution session UUID to persistent install ID.** Anonymous UUID generated on first run, stored at `~/.config/apitesttool/install_id` (mode `0600`), regeneratable via `apitest telemetry reset-id`, deleted when `apitest telemetry delete-request` finalizes. Session UUID is preserved as a per-execution sub-identifier nested inside the install ID's events — both layers ship. This contradicts the prior spec's session-only model; the trade is justified by the conversion-funnel analytics utility, which the session-only model cannot provide. | K, I |
...
| v4-11 | Telemetry CLI emitter shape | **New `internal/telemetry/` package with a dedicated HTTP client.** Does NOT reuse `internal/backend/client.go` — telemetry sends anonymously (no Bearer auth, no refresh-token flow), carries an idempotency key per event for at-most-once delivery, and uses an `apitest.org/telemetry` endpoint distinct from the licensing/billing API host. The shapes don't fit `backend.Client`'s authenticated-RPC pattern cleanly enough to share. | O |
...
| v4-13 | Signing-key plaintext fallback | **Keep deployment-mode-dependent.** Self-hosted may opt out of KMS by setting `APITEST_SIGNING_KEY_MODE=file`. SaaS production builds add a CI lint (`scripts/ci-local.sh` extension) that runs `SELECT COUNT(*) FROM signing_keys WHERE kms_key_id IS NULL` on the production DB connection and fails the build if any row matches. Documented in `docs/COMPLIANCE.md` as a build-time control. | N |

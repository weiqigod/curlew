...
- [v1.1 → v1.2 changelog](#v11--v12-changelog)
...
v1.2 is a **backward-compatible, additive release**. No existing fields are
renamed or removed. A v1.1 consumer will continue to work against a v1.2 stream
by ignoring the new optional field.

**New optional fields in v1.2:**

- `request.start.request_slug` (string, omitempty) — URL-safe identifier derived
  from the request `name`. Paired with `request.end.request_slug`. Used by the W4
  markdown formatter for filenames and sentinel tags.
- `request.end.request_slug` (string, omitempty) — same value as the paired
  `request.start.request_slug`.

**Slug derivation rule (canonical algorithm; changing it requires v2.0):**
...
If the result is empty (the name had no alphanumeric runes after normalization),
`apitest run` rejects the collection at load time with error code
...
| `Héllo` | `hello` |
| `Create user [1/3]` | `create-user-1-3` |
...
- **Schema version** — every event carries `"schema_version": "1.2"`. Consumers
...
|-------|------|----------|-------------|
| **schema_version** | string | yes | Always `"1.2"` |
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml"]}
```

Maximal example (all fields, including v1.1 selection):

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":1,"at_ms":0,"kind":"run.start","started_at":"2026-04-25T10:00:00Z","apitest_version":"1.0.0","cli_args":["run","tests.yaml","--env","staging","--only","Get user"],"collection_file":"tests.yaml","env_name":"staging","selection":["Get user"]}
```
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":0,"kind":"run.error","error":{"category":"parse","message":"unexpected token at line 5"}}
```

Maximal example:

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":3,"kind":"run.error","error":{"category":"parse","code":"PARSE_INVALID_YAML","message":"invalid YAML syntax","hint":"Fix the YAML syntax at the line indicated. Check indentation and quoting.","file":"tests.yaml","line":5}}
```
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","method":"GET","url":"https://api.example.com/users/1"}
```

Maximal example (including v1.2 request_slug):

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":2,"at_ms":1,"kind":"request.start","request_id":"req-1","request_slug":"get-user-by-id","name":"Get user by ID","method":"GET","url":"https://api.example.com/users/1","phase":"test","source_file":"tests.yaml","source_line":12}
```
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","outcome":"passed","duration_ms":42}
```

Example with request_slug and status code (v1.2):

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":4,"at_ms":52,"kind":"request.end","request_id":"req-1","request_slug":"get-user","outcome":"passed","status_code":200,"duration_ms":42,"response_body":"{\"id\":1,\"name\":\"Alice\"}"}
```
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":true}
```

Example with expected and actual (assertion failed):

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":3,"at_ms":45,"kind":"assertion.result","request_id":"req-1","type":"status","passed":false,"expected":"200","actual":"404"}
```
...
```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":7,"at_ms":123,"kind":"run.end","duration_ms":123,"total":2,"passed":2,"failed":0,"skipped":0,"exit_code":0,"event_count":7}
```

Example (failed run, 1 request with assertion failure):

```json
{"schema_version":"1.2","run_id":"a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4","id":5,"at_ms":88,"kind":"run.end","duration_ms":88,"total":1,"passed":0,"failed":1,"skipped":0,"exit_code":1,"event_count":5}
```

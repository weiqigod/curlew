...
import (
	...
	"github.com/peterlindqvist/apitest/internal/loadgen/report"
)
...
func perfCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	case report.FormatJSON:
		if err := writeReportFile(outPath, func(w io.Writer) error {
			return report.WriteJSON(w, metrics, "apitest perf report")
		}); err != nil {
			_, _ = fmt.Fprintf(stderr, "error: %v\n", err)
			return 1
		}
	...
	case report.FormatHTML:
		if err := writeReportFile(outPath, func(w io.Writer) error {
			return report.WriteHTML(w, metrics, "apitest perf report")
		}); err != nil {
			_, _ = fmt.Fprintf(stderr, "error: %v\n", err)
			return 1
		}
...
func printPerfHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest perf <request-file> [options]")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  apitest perf request.yaml --vus 10 --duration 60s")
	_, _ = fmt.Fprintln(w, "  apitest perf request.yaml --vus 10 --duration 60s --output report.json")
	_, _ = fmt.Fprintln(w, "  apitest perf request.yaml --vus 20 --duration 2m --ramp-up 30s --output report.html")
}

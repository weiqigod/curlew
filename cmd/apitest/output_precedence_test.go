...
//
// Each sub-test sets up a temp project root with apitest.yaml (optional
...
func TestOutputPrecedence(t *testing.T) {
	...
	tests := []struct {
		name            string
		projectOut      string // YAML output: block to include in apitest.yaml (empty = omit)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Write apitest.yaml (project config) if a project output block is specified.
			...
			if err := os.WriteFile(filepath.Join(tmp, "apitest.yaml"), []byte(projectYAML), 0o644); err != nil {
				t.Fatalf("write apitest.yaml: %v", err)
			}
...
func TestOutputPrecedence_Events(t *testing.T) {
	...
	writeProject := func(t *testing.T, dir, content string) {
		t.Helper()
		if err := os.WriteFile(filepath.Join(dir, "apitest.yaml"), []byte(content), 0o644); err != nil {
			t.Fatalf("write apitest.yaml: %v", err)
		}
	}
...
func TestOutputPrecedence_Verbosity(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmp, "apitest.yaml"), []byte(
		"project_name: vp\noutput:\n  verbosity: debug\n"), 0o644); err != nil {
		t.Fatalf("write apitest.yaml: %v", err)
	}
	colContent := fmt.Sprintf(

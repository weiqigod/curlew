...
// uiFlags holds parsed `apitest ui` arguments.
type uiFlags struct {
...
func printUIHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest ui [--port <n>] [--env <name>] [--collection <file>] [--no-open] [--no-color]")
	_, _ = fmt.Fprintln(w, "")
	...
	_, _ = fmt.Fprintln(w, "  --port <n>           Listen port (default: ui.port from apitest.yaml, else 8765).")
...
// uiCmdOut implements `apitest ui` (UI_SPECIFICATION.md §2).
func uiCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	srv, err := uiserver.NewServer(uiserver.Options{
		...
		DevProxy:         os.Getenv("APITEST_UI_DEV_PROXY"),
	...
	_, _ = fmt.Fprintf(stdout, "apitest ui listening on %s\n", url)
...
// Resolution order: $APITEST_EDITOR > ui.editor > "" (server falls back to
// `code --goto`).
func resolveUIEditor(uiCfg *config.UIConfig) string {
	if v := os.Getenv("APITEST_EDITOR"); v != "" {
		return v
	}
	if uiCfg != nil {
		return uiCfg.Editor

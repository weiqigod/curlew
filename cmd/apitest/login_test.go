...
func setupLoginEnv(t *testing.T, srvURL string) string {
	t.Helper()
	dir := t.TempDir()
	t.Setenv("APITEST_BACKEND_URL", srvURL)
	t.Setenv("APITEST_CONFIG_DIR", dir)
	t.Setenv("APITEST_FORCE_NO_TTY", "1")
	return dir
}
...
func TestLogin_TTY_OpensBrowserAtVerificationComplete(t *testing.T) {
	...
	opened := stubNoBrowser(t)
	// No --no-browser flag.
	t.Setenv("APITEST_FORCE_NO_TTY", "") // clear so our stubIsTTY controls it
...
func TestLogin_NoTTY_SkipsBrowserEvenWithoutFlag(t *testing.T) {
	...
	stubIsTTY(t, false)
	opened := stubNoBrowser(t)
	t.Setenv("APITEST_FORCE_NO_TTY", "") // ensure our stub controls TTY

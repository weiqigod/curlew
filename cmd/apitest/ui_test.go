...
func TestUICmd_NoProject_Exit5(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "no apitest project found") {
...
func TestUICmd_UnknownEnv_Exit3(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(dir+"/apitest.yaml", []byte("project_name: t\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	oldWd, _ := os.Getwd()
	if err := os.Chdir(dir); err != nil {
...
func TestUICmd_UsageError_Exit1(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "Usage: apitest ui") {

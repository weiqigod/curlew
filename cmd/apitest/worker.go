...
import (
	...
	schedpkg "github.com/peterlindqvist/apitest/internal/worker/schedule"
)

// workerSchedulePullCfg holds flags specific to the --schedule-pull mode.
type workerSchedulePullCfg struct {
	enabled      bool
	backendURL   string        // --backend (or APITEST_BACKEND_URL); defaults to https://api.apitool.dev
	accessToken  string        // resolved from secure storage or APITEST_BACKEND_TOKEN
	...
	teamEnv      string        // --env / APITEST_TEAM_ENV: shared-vault environment for scheduled runs
}
...
func runSchedulePull(sc workerSchedulePullCfg, stdout, stderr io.Writer) int {
	...
	if sc.accessToken == "" {
		_, _ = fmt.Fprintln(stderr, "error: not logged in (run `apitest login` first)")
		return 10
	}
	...
	localTeamConfig := os.Getenv("APITEST_TEAM_CONFIG")
	...
	r := &schedpkg.Runner{
		...
		Queue:  &schedpkg.Queue{}, // Dir resolved lazily to ~/.config/apitesttool/pending-uploads
...
func parseWorkerArgs(args []string) (cfg worker.Config, schedCfg workerSchedulePullCfg, showHelp bool, err error) {
	...
	schedCfg = workerSchedulePullCfg{
		...
		teamEnv:      os.Getenv("APITEST_TEAM_ENV"),
	}
	...
	if schedCfg.enabled {
		...
		if t := os.Getenv("APITEST_BACKEND_TOKEN"); t != "" {
...
func printWorkerHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest worker [options]")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "Run as a distributed worker against an ApiTool coordinator.")
	...
	_, _ = fmt.Fprintln(w, "  --token <token>           Bearer token (or set APITEST_BACKEND_TOKEN)")
	...
	_, _ = fmt.Fprintln(w, "  --backend <url>            Backend base URL (default: $APITEST_BACKEND_URL, then https://api.apitool.dev).")
	...
	_, _ = fmt.Fprintln(w, "  APITEST_COORDINATOR_URL  Default coordinator base URL")
	_, _ = fmt.Fprintln(w, "  APITEST_BACKEND_TOKEN    Default bearer token (also used by --schedule-pull)")
	_, _ = fmt.Fprintln(w, "  APITEST_BACKEND_URL      Default backend URL for --schedule-pull")
	_, _ = fmt.Fprintln(w, "  APITEST_TEAM_ENV         Default shared-vault environment for --schedule-pull")
	_, _ = fmt.Fprintln(w)

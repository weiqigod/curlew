...
import (
	...
	"github.com/peterlindqvist/apitest/internal/discovery"
	"github.com/peterlindqvist/apitest/internal/httpexec"
	"github.com/peterlindqvist/apitest/internal/openapi"
	...
	mdformat "github.com/peterlindqvist/apitest/internal/output/markdown"
	"github.com/peterlindqvist/apitest/internal/parallel"
	"github.com/peterlindqvist/apitest/internal/parser"
	"github.com/peterlindqvist/apitest/internal/prcheck"
	...
	teamtmpl "github.com/peterlindqvist/apitest/internal/vault/teamtemplate"
	"github.com/peterlindqvist/apitest/internal/watch"
...
func runWithWriters(args []string, stdout, stderr io.Writer) int {
	...
	case "internal":
		// Hidden: gated on APITEST_INTERNAL=1. Not listed in help or usageSynopses.
...
func runCmdInner(args []string, stdout, stderr io.Writer) (int, *runner.Summary) {
	...
	if parseErr != nil {
		_, _ = fmt.Fprintln(stderr, "Usage: apitest run <collection-file> [--env <name>] [--env-var VAR ...] [--var key=value ...] [--seed <number>] [--format <type>] [--report <file>] [--only \"<name>\"] [--show-dependencies] [--dry-run] [--parallel] [--confirm-large-dataset] [--report-upload] [--org <name>] [--pr <number>] [--repo <owner/repo>] [--triggered-by <who>] [--git-sha <sha>] [--no-color] [-v] [-vv] [-q]")
	...
	if flags.eventsSet && flags.events != "" {
		...
		eventsCloser = evF
		em, emErr := events.NewEmitter(evF, events.Options{ApitestVersion: version, RunID: runID})
		if emErr != nil {
			_ = evF.Close()
			errEvOut := newStderrPrinterTo(stderr, noColor)
	...
	// Read APITEST_TEAM_CONFIG early so env-file loading can tolerate its presence.
	...
	// When APITEST_TEAM_CONFIG is set or a backend vault cache exists,
	...
	// Load shared vault template: backend cache (base) + APITEST_TEAM_CONFIG (overlay).
	...
	case flags.workers >= 2:
		...
		if coordURL == "" {
			missingCoordErr := fmt.Errorf("--workers requires APITEST_COORDINATOR_URL")
		...
		if token == "" {
			missingTokenErr := fmt.Errorf("--workers requires APITEST_BACKEND_TOKEN")
			_, _ = fmt.Fprintf(stderr, "error: %v\n", missingTokenErr)
			if eventsEmitter != nil {
	...
	if verbosity >= output.VerbosityVerbose {
		if src, val := localeSource(flags, projectCfg, col, environmentLocale); val != "" {
			_, _ = fmt.Fprintf(stderr, "apitest: resolved locale: %s (source: %s)\n", val, src)
		}
	}
...
func handleReportUpload(ctx context.Context, flags runFlags, col *parser.Collection, results []runner.RequestResult, sum *runner.Summary, stdout, stderr io.Writer) int {
	...
	if backendURL == "" {
		_, _ = fmt.Fprintln(stderr, "error: backend URL not configured (set APITEST_BACKEND_URL)")
		return 2
	}
...
func watchCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "Usage: apitest watch <collection-file> [--env <name>] [--var key=value ...] [--format <type>] [--clear] [--no-color] [-v] [-vv] [-q]")
...
func validateCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil || len(patterns) == 0 {
		_, _ = fmt.Fprintln(stderr, "Usage: apitest validate <file|glob> [...] [--format json] [--no-color]")
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "Error: %v\n", err)
		}
		return 1
...
func initCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	_, _ = fmt.Fprintln(stdout, "  apitest.yaml")
	...
	_, _ = fmt.Fprintln(stdout, "  apitest run collections/sample.yaml")
	return 0
}
...
func printInitHelpTo(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  --skill <name>          Scaffold an agent skill template at .claude/skills/apitest/SKILL.md.")
	_, _ = fmt.Fprintln(w, "                          One of: claude.")
	_, _ = fmt.Fprintln(w, "                          --skill claude defaults --output to markdown, appends")
	_, _ = fmt.Fprintln(w, "                          events: .apitest/run.ndjson to the output: block, and")
	_, _ = fmt.Fprintln(w, "                          adds .apitest/ to .gitignore.")
...
func infoCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	if !found {
		_, _ = fmt.Fprintln(stderr, "no apitest project found (no apitest.yaml in current or parent directories)")
		return 5
	}
...
func execCmdOut(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if parseErr != nil {
		_, _ = fmt.Fprintln(stderr, "Usage: apitest exec <url> [--stdin] [-X method] [--dry-run] [--log <file>] [--format <type>] [--non-interactive] [--var key=value ...] [--env-var VAR ...] [--no-color] [-v] [-vv] [-q]")
		...
		if opts.Locale != "" {
			...
			if opts.Verbosity >= output.VerbosityVerbose {
				lopts = append(lopts, variable.WithLocaleWarning(func(m string) {
					_, _ = fmt.Fprintln(stderr, "apitest: "+m)
				}))
			}
		}
...
func vaultListCmdOut(format string, stdout, stderr io.Writer) int {
	...
	if cfg.Secrets == nil {
		if format == "json" {
			...
			_, _ = fmt.Fprintln(stdout)
			_, _ = fmt.Fprintln(stdout, "Add a secrets block to apitest.yaml to configure vault providers.")
		}
		return 0
	}
...
func printVaultHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest vault <subcommand>")
	_, _ = fmt.Fprintln(w)
...
func printHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "apitest — a file-based API testing tool")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintf(w, "Version: %s\n", version)
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "Usage:")
	_, _ = fmt.Fprintln(w, "  apitest <command> [arguments]")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  init [dir]      Initialize a new apitest project (use --output <fmt> to scaffold an output: block; use --skill claude for an agent skill)")
	...
	_, _ = fmt.Fprintln(w, "  --format <type>     Output format: terminal (default), json, tap, junit, html")
	...
	_, _ = fmt.Fprintln(w, "  --report-upload     Upload run results to the ApiTool backend after execution")
	...
	_, _ = fmt.Fprintln(w, "  Env vars:  APITEST_BACKEND_URL   Backend base URL (e.g. http://api.example.com)")
	_, _ = fmt.Fprintln(w, "             APITEST_BACKEND_TOKEN  Bearer token for authentication")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  --coordinator-url <url>  Coordinator base URL (or set APITEST_COORDINATOR_URL)")
	_, _ = fmt.Fprintln(w, "  --org <slug>             Required with --workers")
	_, _ = fmt.Fprintln(w, "  Env vars:  APITEST_COORDINATOR_URL   Coordinator base URL")
	_, _ = fmt.Fprintln(w, "             APITEST_BACKEND_TOKEN     Bearer token for the coordinator API")
	...
	_, _ = fmt.Fprintln(w, "  Honors .apitestignore in the working directory")
	_, _ = fmt.Fprintln(w, "  Example: apitest run \"tests/**/*_test.yaml\"")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  APITEST_TEAM_CONFIG=path   Load a shared vault configuration template")
	...
	_, _ = fmt.Fprintln(w, "                             Cache stored in team_vault.json (~/.config/apitesttool/team_vault.json)")
	_, _ = fmt.Fprintln(w, "  APITEST_VAULT_STUB=1       Use an in-memory stub provider (for local/CI testing)")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "Plugins:")
	_, _ = fmt.Fprintln(w, "  apitest plugins list    Discover plugins from APITEST_PLUGINS and show their")
	_, _ = fmt.Fprintln(w, "                          name, version, and registered hooks.")
	_, _ = fmt.Fprintln(w, "  Env vars:  APITEST_PLUGINS   Colon-separated list of plugin executables or")
	...
	_, _ = fmt.Fprintln(w, "  Set APITEST_PLUGINS=/path/to/plugin[:...] to enable.")
	...
	_, _ = fmt.Fprintln(w, "  apitest.yaml        Project config with global variables (optional, walks up from collection dir)")
...
func importOpenAPICmdOut(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "Usage: apitest import openapi <spec-path> [--output <file>]")
		_, _ = fmt.Fprintf(stderr, "Error: %v\n", err)
		return 1
	}
...
func printPrCheckHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest pr-check [options]")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  APITEST_BACKEND_URL    Backend API base URL (required)")
	_, _ = fmt.Fprintln(w, "  APITEST_BACKEND_TOKEN  Bearer token for authentication (required)")
}

// loadTeamTemplate loads the shared vault template using the backend-cache+overlay loader.
// Returns (nil, nil) when neither backend cache nor APITEST_TEAM_CONFIG is configured.

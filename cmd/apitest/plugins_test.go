...
func TestPluginsList_MissingPathExits2(t *testing.T) {
	...
	t.Setenv("APITEST_PLUGINS", "/does/not/exist/plugin")
...
func TestPluginsList_Help(t *testing.T) {
	...
	if !strings.Contains(stdout, "APITEST_PLUGINS") {
		t.Errorf("expected help output to mention APITEST_PLUGINS, got: %s", stdout)
	}
}
...
// TestPluginsList_Directory verifies that APITEST_PLUGINS pointing to a
...
func TestPluginsList_Directory(t *testing.T) {
	...
	// Point APITEST_PLUGINS at the directory.
	t.Setenv("APITEST_PLUGINS", tmp)
...
// APITEST_PLUGINS set lists the configured plugin.
...
// APITEST_PLUGINS unset prints an empty table.
...
type loopTestPluginDef struct {
	// helloResult is the JSON result for apitest/hello.
...
func TestRun_HookPlugin_TimeoutEmitsWarning(t *testing.T) {
	...
	sp := &loopTestSpawner{plugins: map[string]*loopTestPluginDef{
		pluginPath: {
			...
			stallMethod: "apitest/on_request",
			handler: func(_ string, _ json.RawMessage) (json.RawMessage, error) {
				return json.RawMessage(`{}`), nil
			},
		},
...
func TestRun_HookPlugin_TwoPluginsChainInOrder(t *testing.T) {
	...
	t.Setenv("APITEST_PLUGINS", pathA+string(filepath.ListSeparator)+pathB)
...
// TestRun_PluginsEnvUnset verifies that `apitest run` with APITEST_PLUGINS
// unset runs normally.

// Package main: cmd/apitest/login.go
//
// Implements `apitest login` per the RFC 8628 device-code flow specified
...
var (
	...
	// inject a stub via stubIsTTY or APITEST_FORCE_NO_TTY=1.
	loginIsTTY = stdoutIsTTY
)

// loginCmdOut handles `apitest login [--no-browser]`.
...
func loginExitForError(err error, stderr io.Writer) int {
	...
	case errors.Is(err, backend.ErrExpiredToken):
		_, _ = fmt.Fprintln(stderr, "Code expired; run apitest login again")
		return 4
...
// Honors APITEST_CONFIG_DIR, falling back to ~/.config/apitesttool.
func loginConfigDir() (string, error) {
	return appdir.ResolveConfigDir()
}

// stdoutIsTTY returns true when stdout is a character device.
// APITEST_FORCE_NO_TTY=1 forces false for testing.
func stdoutIsTTY() bool {
	if os.Getenv("APITEST_FORCE_NO_TTY") == "1" {
		return false
	}
...
// printLoginHelpTo writes usage for `apitest login` to w.
func printLoginHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: apitest login [--no-browser]")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "Authenticate via the device-code flow (RFC 8628).")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "When you run `apitest login`, the CLI:")
	_, _ = fmt.Fprintln(w, "  1. Requests a device code from the apitool backend.")
	...
	_, _ = fmt.Fprintln(w, "  4  Device code expired or access denied — run `apitest login` again.")
	_, _ = fmt.Fprintln(w, "  6  Backend returned 5xx — retry later.")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "Env vars:")
	_, _ = fmt.Fprintln(w, "  APITEST_BACKEND_URL=url    Backend base URL (default: https://api.apitool.dev)")

...
func TestPrintPerfHelp_MentionsAllFlags(t *testing.T) {
	...
	if !strings.Contains(out, "Usage: curlew perf") {
		t.Error("printPerfHelp() output missing usage line")
	}
	if !strings.Contains(out, "Examples:") {
		t.Error("printPerfHelp() output missing Examples block")
	}
	if !strings.Contains(out, "curlew perf") || !strings.Contains(out, "--output report.json") {
...
func TestPerfCmd_OutputHTML_WritesFile(t *testing.T) {
	...
	html := string(data)
	if !strings.Contains(html, "<title>curlew perf report</title>") {
		t.Error("HTML output missing <title>curlew perf report</title>")
	}

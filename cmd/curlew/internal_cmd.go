...
// internalCmdOut dispatches `curlew internal <subcommand>`. Hidden behind
// CURLEW_INTERNAL=1; not listed in the top-level help or usageSynopses.
func internalCmdOut(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "Usage: curlew internal <subcommand>")
...
// reachable only through the CURLEW_INTERNAL=1 command gate and deliberately
...
func seedLoginCmdOut(stdout, stderr io.Writer) int {
	cfgDir := os.Getenv("CURLEW_CONFIG_DIR")
	deviceID := os.Getenv("CURLEW_INTERNAL_DEVICE_ID")
	refreshToken := os.Getenv("CURLEW_INTERNAL_REFRESH_TOKEN")
	accessToken := os.Getenv("CURLEW_INTERNAL_ACCESS_TOKEN")
	if cfgDir == "" || deviceID == "" || refreshToken == "" || accessToken == "" {
		_, _ = fmt.Fprintln(stderr,
			"seed-login requires CURLEW_CONFIG_DIR, CURLEW_INTERNAL_DEVICE_ID, "+
				"CURLEW_INTERNAL_REFRESH_TOKEN, and CURLEW_INTERNAL_ACCESS_TOKEN")
		return 1
	}
...
func backendProbeCmdOut(args []string, stdout, stderr io.Writer) int {
	...
	cfgDir, err := os.MkdirTemp("", "curlew-probe-")
	if err != nil {
		_, _ = fmt.Fprintln(stderr, err)
		return 1
	}

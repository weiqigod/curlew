...
func testPlaybookExit2(t *testing.T) {
	...
	// Per playbook: agent reads .curlew/run.ndjson. The guard rail emits a
	...
	if !ndjsonContainsRunEnd(t, events, 2) {
		t.Errorf("playbook says agent reads .curlew/run.ndjson; run.end with exit_code=2 not found in %s", events)
	}
}
...
func TestSkillClaude_MultiFile_PlaybookHasCELErrorCodes(t *testing.T) {
	...
	s := string(body)
	for _, substr := range []string{"ERR_CEL_PARSE", "ERR_CEL_TYPE", "curlew validate"} {
		if !strings.Contains(s, substr) {
...
func TestSkillClaude_MultiFile_TriggerPhrasesPreserved(t *testing.T) {
	...
	s := string(body)
	for _, phrase := range []string{"run the test", "hit the staging API", "use curlew to"} {

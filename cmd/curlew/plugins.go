...
// buildHookDispatcher loads plugins from CURLEW_PLUGINS, keeps their channels
...
// CURLEW_PLUGINS is empty (no error — just no-op).
...
// pluginsCmdOut dispatches `curlew plugins <subcommand>`.
...
// pluginsListCmdOut loads all plugins from CURLEW_PLUGINS and prints a table.
...
func printPluginsHelpTo(w io.Writer) {
	_, _ = fmt.Fprintln(w, "Usage: curlew plugins <subcommand>")
	_, _ = fmt.Fprintln(w)
	...
	_, _ = fmt.Fprintln(w, "  CURLEW_PLUGINS   Colon-separated (semicolon-separated on Windows) list of plugin")

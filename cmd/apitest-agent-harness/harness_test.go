...
// buildApitestBinary compiles the real apitest binary into a temp directory and
// returns its path. The binary is rebuilt fresh for every test run to avoid
// stale-build flakiness.
func buildApitestBinary(t *testing.T) string {
	t.Helper()
	binary := filepath.Join(t.TempDir(), "apitest")
	// Locate the cmd/apitest package relative to this file.
	_, thisFile, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("runtime.Caller failed")
	}
	// thisFile = .../cmd/apitest-agent-harness/harness_test.go
	// pkg = .../cmd/apitest
	pkg := filepath.Join(filepath.Dir(filepath.Dir(thisFile)), "apitest")

	cmd := exec.Command("go", "build", "-o", binary, pkg)
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("build apitest binary: %v\n%s", err, out)
	}
	return binary
}
...
// runApitestForScenario executes apitest run against a scenario's collection
...
func runApitestForScenario(t *testing.T, binary string, s Scenario) (eventsPath string, exitCode int) {
	t.Helper()
	tmp := t.TempDir()
...
// TestHarness_AllScenarios builds the apitest binary, discovers all scenario
...
func TestHarness_AllScenarios(t *testing.T) {
	if testing.Short() {
		t.Skip("harness builds the apitest binary; skipping in -short mode")
	}
	binary := buildApitestBinary(t)

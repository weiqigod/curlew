// File-based KEK implementation of ITeamVaultKeyProvider for self-hosted deployments.
// Refs M18-009 (v4-12).
...
/// <summary>
/// <see cref="ITeamVaultKeyProvider"/> implementation backed by a 32-byte AES-256 KEK file.
...
public sealed class FileTeamVaultKeyProvider : ITeamVaultKeyProvider
{
    ...
    /// non-vault code paths (e.g., health checks, internal endpoints) can start without
    ...
    public Task<TeamVaultEncryptionResult> EncryptAsync(byte[] plaintext, CancellationToken ct = default)
    {
        ...
        return Task.FromResult(new TeamVaultEncryptionResult(blob, KidValue));
    }
    ...
    {
        if (kid != KidValue)
            throw new TeamVaultDecryptException("file",
        ...
        {
            throw new TeamVaultDecryptException("file", ex);
        }
    }
    ...
    private byte[] LoadKek()
    {
        ...
        if (string.IsNullOrEmpty(_kekPath) || !File.Exists(_kekPath))
            throw new InvalidOperationException(
                $"TeamVault KEK file not found at '{_kekPath}'. Set ApiTool:TeamVault:Encryption:KeyProvider:File:KekPath " +
                $"(env TEAMVAULT__KEY_PROVIDER=file TEAMVAULT__KEK_PATH=<path>) to a 32-byte file (mode 0600).");

        var bytes = File.ReadAllBytes(_kekPath);
        if (bytes.Length != 32)
            throw new InvalidOperationException(
                $"TeamVault KEK file at '{_kekPath}' is {bytes.Length} bytes; required size is exactly 32 bytes (AES-256).");

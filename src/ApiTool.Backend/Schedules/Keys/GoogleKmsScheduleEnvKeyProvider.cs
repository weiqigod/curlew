// KMS-backed IScheduleEnvKeyProvider for SaaS deployments (HSM-tier key custody).
// Refs M18-009 (v4-12).
...
/// <summary>
/// <see cref="IScheduleEnvKeyProvider"/> implementation that wraps per-row DEKs using Google Cloud KMS.
...
/// Refs M18-009 (v4-12).
/// </summary>
public sealed class GoogleKmsScheduleEnvKeyProvider(IKmsClient kms, IOptions<ScheduleEnvEncryptionOptions> opts) : IScheduleEnvKeyProvider
{
    private readonly string _kmsKeyId = opts.Value.KeyProvider.Kms.KmsKeyId;

    /// <inheritdoc/>
    public async Task<ScheduleEnvEncryptionResult> EncryptAsync(byte[] plaintext, CancellationToken ct = default)
    {
        try
        {
            ...
            return new ScheduleEnvEncryptionResult(blob, _kmsKeyId);
        }
        ...
        {
            throw new ScheduleEnvDecryptException("kms", ex);
        }
    }

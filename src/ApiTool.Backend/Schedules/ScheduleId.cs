...
public static class ScheduleId
{
    ...
    /// <summary>Formats a <see cref="Guid"/> as a <c>sched_</c>-prefixed hex string.</summary>
    public static string Format(Guid id) => Prefix + id.ToString("N");

    /// <summary>
    /// Attempts to parse a <c>sched_</c>-prefixed wire-format id back to a <see cref="Guid"/>.
    /// </summary>
    /// <param name="value">The wire-format string (e.g. <c>sched_abc123…</c>).</param>

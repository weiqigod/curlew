...
| Coverage `cmd/apitest` | 81.6% | Meets >= 80% threshold |
...
| 3 | APITEST_PLUGINS is a directory → all executables loaded | `TestPluginsList_Directory`, `TestDiscover/directory_expands_to_sorted_executables` | PASS |
...
| 7 | `--help` documents APITEST_PLUGINS | `TestPluginsList_Help`, `TestPluginsList_HelpArg` | PASS |
...
| 2 | Observable output works as specified | `APITEST_PLUGINS=/tmp/apitest-hello-plugin ./apitest plugins list` produces correct table | PASS |
| 3 | Test coverage >= 80% | internal/plugin 87.9%, cmd/apitest 81.6%, total 86.7% | PASS |
| 4 | No build warnings or lint errors | `go build ./cmd/apitest` clean; `golangci-lint run` 0 issues | PASS |
| 5 | `docs/plugins.md` documents handshake and APITEST_PLUGINS | File present, 6043 bytes, covers all required sections | PASS |
| 6 | Smoke test exercises `apitest plugins list` with testdata plugin | `=== Plugins (M5-017) ===` block in smoke/run.sh passes all 4 assertions | PASS |
...
| 265e09c | docs(plugin): add docs/plugins.md with handshake schema and APITEST_PLUGINS docs |
| ea83d70 | feat(plugin): add hello-plugin fixture and real-fixture integration test |
| f9ad93b | feat(cli): add apitest plugins list subcommand |
| 2a5fdc7 | test(cli): add failing tests for apitest plugins list command |
...
| 31ad653 | feat(plugin): implement APITEST_PLUGINS discovery with executable checks |
| e115c97 | test(plugin): add failing tests for APITEST_PLUGINS discovery |
...
| `cmd/apitest/plugins.go` | created |
| `cmd/apitest/plugins_test.go` | created |
| `cmd/apitest/main.go` | modified — plugins case + help text |

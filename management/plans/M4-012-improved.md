...
| 3 | High | `smoke/run.sh` missing stack lifecycle idempotency assertion (DoD item not met) | Added `=== Stack idempotency (M4-012) ===` section at end of `smoke/run.sh` gated by `CURLEW_MANAGE_STACK=1`; prints SKIP with instructions when stack not available, runs COUNT_BEFORE/COUNT_AFTER org-count check when stack is up | ✓ SKIP path passes in CI |
...
| 5 | Medium | gofumpt formatting violation in `cmd/curlew/main.go` `runFlags` struct field alignment | Applied `~/go/bin/gofumpt -w cmd/curlew/main.go` | ✓ golangci-lint 0 issues |

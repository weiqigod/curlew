...
## Overview
Introduce a shared `usageSynopsis(cmd string) string` helper so every error-recovery site in `cmd/apitest/` writes a one-line "Usage: …" synopsis to stderr instead of dumping the full help block to stdout. Explicit `--help` invocations continue to write the full help to stdout, exit 0.
...
|----------|------|-----------|------------------|
| `printHelpTo` | `cmd/apitest/main.go:3127` | `func(w io.Writer)` | `apitest <command> [arguments]` (preceded by a bare `Usage:` line) |
| `printPluginsHelpTo` | `cmd/apitest/plugins.go:155` | `func(w io.Writer)` | `Usage: apitest plugins <subcommand>` |
| `printPerfHelpTo` | `cmd/apitest/perf.go:264` | `func(w io.Writer)` | `Usage: apitest perf <request-file> [options]` |
| `printWorkerHelpTo` | `cmd/apitest/worker.go:142` | `func(w io.Writer)` | `Usage: apitest worker [options]` |
...
| `perf_test.go:129-148` `TestPrintPerfHelp_MentionsAllFlags` | Calls `printPerfHelpTo(&buf)` directly and asserts `"Usage: apitest perf"` | Unchanged — direct call still emits full help |
...
**No existing test breaks.** The only observable behaviour change is: `apitest <unknown>`, `apitest plugins <unknown>`, `apitest perf <bad-flag>`, `apitest worker <bad-flag>`, and `apitest import <unknown-format>` stop writing the full help block to stdout on error paths. Stdout is empty on those error paths, and a one-line synopsis appears on stderr alongside the error.

### Intentionally out of scope

- `cmd/apitest/license.go:31-33` has the same `Unknown license flag → printLicenseHelpTo(stdout)` anti-pattern. **Not in the task's affected-sites list** — leave untouched to respect the task boundary. A follow-up task can harmonise it.
- The `import openapi <bad-args>` path at `main.go:3277-3283` already emits a `Usage: apitest import openapi ...` line to stderr (inline, not via the shared helper). The task lists only the top-level `import` entry point and the "Unknown import format" branch, so the inner `import openapi` parse error site is out of scope.
...
// keyed by the subcommand name ("" = top-level apitest). Keep each value in
...
var usageSynopses = map[string]string{
    "":        "Usage: apitest <command> [arguments]",
    "run":     "Usage: apitest run <collection-file> [options]",
    "perf":    "Usage: apitest perf <request-file> [options]",
    "worker":  "Usage: apitest worker [options]",
    "plugins": "Usage: apitest plugins <subcommand>",
    "import":  "Usage: apitest import <format> <spec-path>",
}

// usageSynopsis returns the one-line "Usage: ..." synopsis for the named
// subcommand ("" for the top-level apitest command). Returns the top-level
...
Note: the top-level `printHelpTo` emits a bare `Usage:` line followed by `  apitest <command> [arguments]` on the next line (not a single `Usage: apitest ...` line). For the synopsis, we use the single-line canonical form `Usage: apitest <command> [arguments]` — it matches every behaviour-bullet phrasing in the task YAML and is what the `TestStreamHelp` tests will look for. The help-first-line test for the top-level only needs to confirm both tokens (`Usage:` and `apitest <command>`) appear in the full help output.
...
|------|--------|-------------|
| `cmd/apitest/main.go` | modify | Add `usageSynopses` map + `usageSynopsis` function immediately above `printHelpTo` (around line 3126). |
| `cmd/apitest/main_test.go` | modify | Append `TestUsageSynopsis_MatchesPrintHelpFirstLine`, `TestUsageSynopsis_UnknownKeyFallsBackToTopLevel`, `TestUsageSynopsis_ImportStartsWithUsage`. |
...
| File | Action | Description |
|------|--------|-------------|
| `cmd/apitest/main.go` | modify | Replace the `printHelpTo(stdout)` dump on the `default:` branch of the top-level switch with a stderr synopsis line. |
| `cmd/apitest/stream_help_test.go` | create | New regression test file with `TestStreamHelp` covering the top-level paths. |
...
```go
// cmd/apitest/stream_help_test.go
package main
...
func TestStreamHelp(t *testing.T) {
    t.Run("top_level_unknown_command", func(t *testing.T) {
        ...
        if !strings.Contains(stderr.String(), "Usage: apitest") {
            t.Errorf("stderr missing 'Usage: apitest' synopsis: %q", stderr.String())
        }
    })
...
| File | Action | Description |
|------|--------|-------------|
| `cmd/apitest/plugins.go` | modify | Replace `printPluginsHelpTo(stdout)` on the unknown-subcommand branch with `Fprintln(stderr, usageSynopsis("plugins"))`. |
| `cmd/apitest/stream_help_test.go` | modify | Add `plugins_unknown_subcommand` and `plugins_explicit_help` subtests to `TestStreamHelp`. |
...
t.Run("plugins_unknown_subcommand", func(t *testing.T) {
    ...
    if !strings.Contains(stderr.String(), "Usage: apitest plugins") {
...
| File | Action | Description |
|------|--------|-------------|
| `cmd/apitest/perf.go` | modify | Replace `printPerfHelpTo(stdout)` on the parse-error branch with `Fprintln(stderr, usageSynopsis("perf"))`. |
| `cmd/apitest/stream_help_test.go` | modify | Add `perf_malformed_flag` and `perf_explicit_help` subtests. |
...
t.Run("perf_malformed_flag", func(t *testing.T) {
    ...
    if !strings.Contains(stderr.String(), "Usage: apitest perf") {
        t.Errorf("stderr missing synopsis: %q", stderr.String())
    }
})
...
t.Run("perf_explicit_help", func(t *testing.T) {
    ...
    if !strings.Contains(stdout.String(), "Usage: apitest perf") {
...
| File | Action | Description |
|------|--------|-------------|
| `cmd/apitest/worker.go` | modify | Replace `printWorkerHelpTo(stdout)` on the parse-error branch with `Fprintln(stderr, usageSynopsis("worker"))`. |
| `cmd/apitest/stream_help_test.go` | modify | Add `worker_malformed_flag` and `worker_explicit_help` subtests. |
...
t.Run("worker_malformed_flag", func(t *testing.T) {
    ...
    if !strings.Contains(stderr.String(), "Usage: apitest worker") {
        t.Errorf("stderr missing synopsis: %q", stderr.String())
    }
})
...
t.Run("worker_explicit_help", func(t *testing.T) {
    ...
    if !strings.Contains(stdout.String(), "Usage: apitest worker") {
        t.Errorf("stdout missing full help: %q", stdout.String())
    }
})
```
...
| File | Action | Description |
|------|--------|-------------|
| `cmd/apitest/main.go` | modify | `importCmdOut` emits `usageSynopsis("import")` on both the no-args and unknown-format branches. |
| `cmd/apitest/stream_help_test.go` | modify | Add `import_no_args` and `import_unknown_format` subtests. |
...
func importCmdOut(args []string, stdout, stderr io.Writer) int {
    if len(args) == 0 {
        _, _ = fmt.Fprintln(stderr, "Usage: apitest import <format> <spec-path> [--output <file>]")
...
Note: the pre-existing literal was `Usage: apitest import <format> <spec-path> [--output <file>]` — longer than the shared synopsis. The task's behaviour bullet specifies the shorter form `Usage: apitest import <format> <spec-path>` exactly, so dropping the `[--output <file>]` fragment is explicitly what the task asks for. Option discovery still happens via `--help` on the concrete format (`apitest import openapi --help`).
...
t.Run("import_no_args", func(t *testing.T) {
    ...
    if !strings.Contains(stderr.String(), "Usage: apitest import") {
        t.Errorf("stderr missing synopsis: %q", stderr.String())
    }
})
...
```
- `apitest <unknown-command>`, `apitest plugins <unknown-subcommand>`, `apitest perf <malformed-flag>`, `apitest worker <malformed-flag>`, and `apitest import <unknown-format>` now emit a one-line `Usage: apitest ...` synopsis to stderr instead of dumping the full help block to stdout; stdout is empty on every error path. Explicit `apitest --help` / `apitest <subcommand> --help` continue to write the full help text to stdout and exit 0. New helper `usageSynopsis(cmd string) string` in `cmd/apitest/main.go` keeps the short synopsis string in one place, and a synchronization test (`TestUsageSynopsis_MatchesPrintHelpFirstLine`) asserts each synopsis still appears verbatim in its corresponding `print*HelpTo` full-help output. Regression guard: `cmd/apitest/stream_help_test.go::TestStreamHelp` exercises every (subcommand × invocation-style) cell via `runWithWriters`. (M7-003)
```
...
|-----------|--------------|--------|----------------|
| `cmd/apitest/main_test.go` | `TestCLIIntegration/"unknown command"` | still passes | none (only asserts `"Unknown command"` on stderr) |
| `cmd/apitest/main_test.go` | `TestCLIIntegration/"no args shows help"`, `"help flag"` | still passes | none (stdout still has full help) |
| `cmd/apitest/main_test.go` | `TestRun_watch_command_recognized` | still passes | none |
| `cmd/apitest/main_test.go` | `TestImportCmd_UnknownFormat` | still passes | none (exit-code only) |
| `cmd/apitest/main_test.go` | **new** `TestUsageSynopsis_*` (3 tests) | new | add in Step 1 |
| `cmd/apitest/perf_test.go` | `TestPrintPerfHelp_MentionsAllFlags` | still passes | none (direct print* call) |
| `cmd/apitest/perf_test.go` | `TestPerfCmd_InvalidVUsExitCode2` etc. | still passes | none (exit-code only) |
| `cmd/apitest/plugins_test.go` | `TestPluginsCmd_UnknownSubcmd`, `TestPluginsList_HelpArg`, `TestPluginsList_UnknownFlag` | still passes | none |
| `cmd/apitest/worker_test.go` | all | still passes | none |
| `cmd/apitest/stream_help_test.go` | **new** `TestStreamHelp` (11 subtests) | new | create in Step 2; extend in Steps 3/4/5/6 |
...
/tmp/apitest unknown-command > /tmp/out.txt 2> /tmp/err.txt || true
...
/tmp/apitest --help > /tmp/out.txt 2> /tmp/err.txt

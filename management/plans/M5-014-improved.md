...
| 3 | Medium | Missing `TestLicenseExport_TamperedBundle` for Behavior 6: tamper bundle after export, validate via `APITEST_LICENSE_BUNDLE`, expect rc=6 + `signature_invalid` | Added `TestLicenseExport_TamperedBundle` in `cmd/apitest/license_test.go` — exports a bundle, extracts it, corrupts the JWT signature in `license.json`, sets `APITEST_LICENSE_BUNDLE`, runs `license --validate`, asserts rc=6 and `"signature_invalid"` in stderr | ✓ tests pass |
...
| 6 | Low | Pre-existing license tests (6 tests in `cmd/apitest/license_test.go`) do not call `t.Setenv("APITEST_LICENSE_BUNDLE", "")`, risking cross-contamination if the env var is set in a developer's shell | Added `t.Setenv("APITEST_LICENSE_BUNDLE", "")` to all pre-existing tests that set `APITEST_CONFIG_DIR` but did not clear the bundle env var | ✓ tests pass |

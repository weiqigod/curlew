...
|---|----------|---------|------------|----------|
| 1 | Medium | `TestRedact_BodyAcrossFormats` missing `junit_format` subtest | Added `junit_format` subtest in `cmd/apitest/main_test.go`; elevated tier to `TierProfessional` to bypass the feature gate so redaction is proven for JUnit XML output | ✓ tests pass |

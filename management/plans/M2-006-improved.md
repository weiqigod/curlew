...
| 3 | Low | No binary-level vault list test for GCP and 1Password (behavior 5 gap) | Added `TestVaultCmd_solo_tier_gcp` and `TestVaultCmd_solo_tier_1password` to `cmd/apitest/main_test.go` asserting provider name and key count appear in `vault list` output | ✓ tests pass |

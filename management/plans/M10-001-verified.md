...
$ curlew init --skill claude && test -f .claude/skills/curlew/SKILL.md && echo "skill scaffolded"
skill scaffolded
...
<!-- curlew-skill: claude v1.0 (curlew 0.1.0-dev) -->

# --skill claude --output json adds both format and events
$ curlew init --skill claude --output json && grep -A 5 '^output:' curlew.yaml | grep -E 'format: json|events: \.curlew/run\.ndjson'
  format: json
  events: .curlew/run.ndjson

# Bare init has no events
$ curlew init && grep events curlew.yaml || echo "no events reference"
no events reference

# Unknown skill exits 3 and names claude
$ curlew init --skill madeup 2>err.log; echo $?; grep -E 'claude' err.log
3
Error: unknown --skill value "madeup" (supported: claude)

# --help documents --skill flag
$ curlew init --help | grep -E '^\s*--skill'
  --skill <name>          Scaffold an agent skill template at .claude/skills/curlew/SKILL.md.

# Pre-existing responses/ preserved
$ mkdir responses && echo "preexisting" > responses/keep.md && curlew init --skill claude && test -f responses/keep.md && echo "responses preserved"
responses preserved
...
| 4 | .gitignore extended with .curlew/; bare init still only .env | `TestInit_SkillClaude_ExtendsGitignore`, `TestEnsureGitignoreMultipleEntries` | PASS |
| 5 | Embedded SKILL.md rendered with {{curlew_version}} substituted | `TestInit_SkillClaude_VersionInTemplate`, `TestRender_SkillClaude_SubstitutesVersion` | PASS |
...
| 16 | Coverage >= 80% (scaffold, cmd/curlew, templates) | 86.6% / 81.3% / 90.9% | PASS |
...
| 03c5642 | feat(scaffold): add SkillName/CurlewVersion to Options, installSkill helper |
...
| File | Action | Lines +/- |
|------|--------|-----------|
| `cmd/curlew/main.go` | modified | +74/-5 |
| `cmd/curlew/main_test.go` | modified | +152/-0 |
...
| `templates/skills/claude/curlew/SKILL.md` | added | +17 |

...
4. **`GitHubAppOptions` is a brand-new options class.** Bound under section `ApiTool:GitHubApp`. Spelled `GITHUB_APP__*` in env vars (per task observable). It is **not** a sub-section of `KeyProviderOptions` because the two are unrelated keys.
...
| `src/ApiTool.Backend/GitHub/GitHubAppOptions.cs` | create | Options class bound from `ApiTool:GitHubApp` |
...
The factory must now provide a real RSA PEM in a temp dir and configure `ApiTool:GitHubApp:*` so `FileGitHubAppKeyProvider` can sign in tests:
...
ASPNETCORE_ENVIRONMENT=Testing \
  ...
  GITHUB_APP__SLUG=apitool-checks-test \

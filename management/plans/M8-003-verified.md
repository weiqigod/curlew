...
ok  github.com/weiqigod/curlew/internal/schema (cached)
...
https://raw.githubusercontent.com/peterlindqvist/curlew/main/schemas/project-v1.json

$ ./curlew schema | jq -r .title
Curlew Collection v1

$ ./curlew run examples/output-block/collection.yaml | jq '.' | head -5
{
...
$ ./curlew run examples/output-block/bad-format.yaml; echo "Exit: $?"
...
| 10 | `curlew schema --project` emits project schema; no flag emits collection schema | `TestSchemaCmd` (2 rows) | PASS |
| 11 | `scaffold.Init` emits active `output:` block and validates against project schema | `TestInit`, `TestSchema_scaffolded_curlew_yaml_validates` | PASS |
...
| 11 | `TestSchema_scaffolded_curlew_yaml_validates` passes | PASS | PASS |
...
| 15 | `curlew schema --project` / no-flag backward compat | Observable verified | PASS |
...
| 20 | Coverage >= 80% in all listed packages | min 81.2% (`cmd/curlew`) | PASS |
...
| 0214235 | feat(cmd): curlew schema --project emits project schema |
...
| `cmd/curlew/main.go` | modified — `runFlags` parallel bools, `resolveOutputPrecedence`, `--project` flag, `--project-name` flag, two-phase events/gate |
| `cmd/curlew/output_precedence_test.go` | created — `TestOutputPrecedence*` |
| `cmd/curlew/main_test.go` | modified — `TestSchemaCmd` rows, `TestInitCmd` row |
| `internal/scaffold/scaffold.go` | modified — `curlewYAML` emits `output:` block |

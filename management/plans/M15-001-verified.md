...
```
# Free tier rejects APITEST_PLUGINS (exit 6):
$ APITEST_TIER=free APITEST_PLUGINS=./cmd/apitest/testdata/plugins/hello ./apitest plugins list 2>&1; echo "exit=$?"
✗ Feature requires upgrade

  Plugin loading via APITEST_PLUGINS requires Enterprise tier
...
# Empty APITEST_PLUGINS at free tier (header-only table, exit 0):
$ APITEST_TIER=free ./apitest plugins list; echo "exit=$?"
NAME  VERSION  HOOKS
exit=0
```

Expected: registered Description ("Plugin loading via APITEST_PLUGINS requires Enterprise tier") and Workaround in stderr, exit 6 for free tier. Header-only table + exit 0 when env unset.
Result: MATCH
...
| 2 | APITEST_PLUGINS unset → no gate check, (nil, noop, nil) returned | `TestRun_PluginGate_NoCheckWhenEnvUnset`, `TestPluginsList_GateSkippedWhenEnvUnset` | PASS |
| 3 | APITEST_PLUGINS set + Free/Solo/Professional/Team → buildHookDispatcher returns *GateError before LoadForRun | `TestRun_PluginGate_RejectsBelowEnterprise` (spawned==0) | PASS |
| 4 | APITEST_PLUGINS set + Enterprise → host.LoadForRun invoked, dispatcher returned | `TestRun_HookPlugin_OnRequestReceivesRequest` et al. | PASS |
| 5 | APITEST_PLUGINS set + Free/Solo/Professional/Team → pluginsListCmdOut exits 6 before host.Load | `TestPluginsList_GateRejectsBelowEnterprise` (spawned==0) | PASS |
| 6 | APITEST_PLUGINS set + Enterprise → host.Load invoked, table rendered | `TestPluginsList_GateAllowsEnterprise` | PASS |
...
| 8 | docs/MANUAL.md plugin tier-gate paragraph verified to match enforcement | MANUAL.md explicitly states APITEST_PLUGINS, Enterprise tier, exit 6 | PASS |

## Definition of Done

| # | Item | Evidence | Status |
|---|------|----------|--------|
| 1 | All behavior tests pass | `go test ./internal/auth/... ./cmd/apitest/... -run "Plugin\|Gate"` — 8 test functions, all PASS | PASS |
| 2 | `go test ./...` passes | ci-local.sh: all packages ok | PASS |
| 3 | `go test -cover ./internal/auth/... ./cmd/apitest/... >= 80%` | 89.6% + 81.5% (total 81.8%) | PASS |
...
| 9 | `cmd/apitest/plugins.go` gates both `buildHookDispatcher` and `pluginsListCmdOut` | Lines 44–47 and 119–131 | PASS |
...
| 11 | `docs/MANUAL.md` plugin tier-gate paragraph verified and sharpened | Exit code and APITEST_PLUGINS explicitly stated | PASS |
...
| 8a54f2c3 | test(cli): add APITEST_TIER=enterprise to existing plugin happy-path tests |
| ccf6fe1d | docs(manual): sharpen plugin tier-gate paragraph with exit code and enforcement note |
| d5cbd2a6 | chore(smoke): add APITEST_TIER=enterprise to plugin sections, add free-tier reject test |
...
| `cmd/apitest/plugins.go` | modified | Gate in `buildHookDispatcher` (lines 44–47) and `pluginsListCmdOut` (lines 119–131) |
| `cmd/apitest/plugins_test.go` | modified | 5 new gate tests; 11 existing tests patched with `APITEST_TIER=enterprise` |
| `cmd/apitest/main.go` | modified | `errors.As` mapping for gate error at `buildHookDispatcher` callsite (~line 1241) |
| `cmd/apitest/discovery_run.go` | modified | `writeGateForFormat` used in run-path gate rendering |
| `docs/MANUAL.md` | modified | Plugin tier-gate paragraph sharpened with exit code and enforcement note |
| `smoke/run.sh` | modified | `APITEST_TIER=enterprise` added to 4 happy-path invocations; free-tier reject block added |

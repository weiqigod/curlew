...
| 2 | Medium | Behavior #7 (800 ms latency SLA on 5M-row fixtures) had no test coverage | Added `DashboardLatencyBenchTests.cs` with two `[SkippableFact]` tests guarded by `CURLEW_RUN_DASHBOARD_LATENCY_BENCH=1` env var. Stats test also verifies `EXPLAIN ANALYZE` references `IX_Results_OrgId_CreatedAt`. | ✓ benchmark tests skip cleanly; non-benchmark tests still pass |

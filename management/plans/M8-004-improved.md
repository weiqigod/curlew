...
| 3 | High | No integration test for `apitest validate` with duplicate-name rejection | Added `TestValidateCmd_DuplicateRequestNames`. | ✓ tests pass |
...
| # | Severity | Finding | Fix Applied | Verified |
|---|----------|---------|------------|----------|
| 1 | High | `filterShowDepsItems` at 0% test coverage — `--only` + `--show-dependencies` had no test | Added `TestRun_ShowDeps_OnlyFilter` in `cmd/apitest/main_test.go` with three sub-cases: selected request in waves, union of two, no-match exit 3. `filterShowDepsItems` is now at 100%. | ✓ tests pass |
...
| 2 | Low | No test asserted `run.end.total` correctness on the no-match events path. | Added `TestRun_OnlyNoMatch_EventsTotal` in `cmd/apitest/main_test.go`: emits events with `--events`, finds `run.end`, asserts `total==0` (fixture has no setup/teardown) and `exit_code==3`. | ✓ tests pass |

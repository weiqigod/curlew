...
description: Final verification report for the curlew worker --schedule-pull slice
type: project
---
...
| `go test ./...` (full) | PASS | Cached across packages; M16-011 targeted re-run with `-count=1` passes (`internal/worker/schedule`, `internal/backend`, `cmd/curlew`). |
...
| Coverage | PASS | `internal/worker/schedule` 84.3%, `cmd/curlew` 81.5%, `internal/backend` 83.9% — all exceed the ≥80% threshold. |
...
```text
$ ./curlew worker --help | grep -E "schedule-pull|--once"
  ...
  CURLEW_BACKEND_TOKEN    Default bearer token (also used by --schedule-pull)
  CURLEW_BACKEND_URL      Default backend URL for --schedule-pull
```
...
| 9 | `worker --help` documents `--schedule-pull` | Smoke `=== Schedule Pull Help (M16-011) ===`; manual `curlew worker --help` (see Observable section above) | PASS |
...
| 2 | Observable command works | `./curlew worker --help | grep schedule-pull` shows flag with description and future-extension reservation | PASS |
| 3 | Test coverage ≥ 80% on new code | `internal/worker/schedule` 84.3%, `cmd/curlew` 81.5%, `internal/backend` 83.9% | PASS |
| 4 | No build warnings or lint errors | `go build ./cmd/curlew` clean; `golangci-lint run` 0 issues | PASS |
| 5 | Help text updated for new flags/subcommands | `curlew worker --help` documents `--schedule-pull`, `--once`, `--backend`, `--poll-interval`; reserves multi-mode combo | PASS |
...
- `cmd/curlew/worker.go` — `--schedule-pull` flag wired, `parseWorkerArgs` + tests extended.
...
| bb2cbcf7 | feat(cli): wire --schedule-pull flag into curlew worker command |
...
- `cmd/curlew/{worker.go,worker_schedule_test.go,worker_test.go}` (modified)

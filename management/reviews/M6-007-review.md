...
| Code Organization | PASS | `cmd/apitest-agent-harness/` correctly scoped as test-only. `harness.go` uses `//go:build never` to exclude from release builds. `contract.go` in `package harness_test` coexists with `harness.go` in `package harness` cleanly. `internal/` boundaries respected. No circular dependencies. |
...
- `cmd/apitest-agent-harness`: **93.9%** — above 80% threshold

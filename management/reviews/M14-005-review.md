...
- `cmd/curlew`: **81.7%** — above 80% threshold
...
|----------|--------|-------|
| `go test ./cmd/curlew/... -run TestLogin` passes with ≥8 tests | PASS | 14 tests pass |
...
| Help text registers `curlew login --no-browser` and explains device-code UX | PASS | `printLoginHelpTo` covers usage, options, exit codes, env vars |
...
| `docs/SPECIFICATION.md:8205–8222` cited in `cmd/curlew/login.go` header | PASS | Line 3 of login.go references the spec anchor |
...
| 5 | expired_token → exit 4 + "Code expired; run curlew login again" | `TestLogin_Polling_ExpiredToken_ExitFour` |

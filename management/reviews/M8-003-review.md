...
| `curlew schema --project` emits project schema; no flag emits collection schema | `TestSchemaCmd` (`project_flag_emits_project_schema`, `no_flag_emits_collection_schema`) |
| `scaffold.Init` emits active `output:` block and validates against project schema | `TestInit` ("curlew.yaml contains active output block"), `TestSchema_scaffolded_curlew_yaml_validates` |

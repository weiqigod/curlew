...
| Naming | PASS | All exported C# symbols (`ResultsEndpoints`, `IngestResultResponse`, `ListResultsResponse`) have doc comments. No stuttering. TypeScript helpers (`lookupOrgGuid`, `triggerSamlLogin`, `seedAuthCookie`, `runCurlew`) are clearly named with JSDoc comments. Package/file names follow conventions. |

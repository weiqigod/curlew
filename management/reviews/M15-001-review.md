...
| Test Quality | PASS | All 8 behaviors covered. Four-tier matrix tested at both call sites. Workaround text (`"built-in signers"`) asserted at CLI level. Spawner-never-invoked check in both `plugins list` and `run` paths. Existing happy-path tests patched with `APITEST_TIER=enterprise`. `smoke/run.sh` extended with free-tier rejection block. |
...
| APITEST_PLUGINS unset → no gate check, (nil, noop, nil) returned | `TestRun_PluginGate_NoCheckWhenEnvUnset`, `TestPluginsList_GateSkippedWhenEnvUnset` |
| APITEST_PLUGINS set + non-Enterprise tier → buildHookDispatcher returns *GateError before LoadForRun | `TestRun_PluginGate_RejectsBelowEnterprise` (spawned==0) |
| APITEST_PLUGINS set + Enterprise → host.LoadForRun invoked, dispatcher returned | `TestRun_HookPlugin_OnRequestReceivesRequest` et al. |
| APITEST_PLUGINS set + non-Enterprise tier → pluginsListCmdOut exits 6 before host.Load | `TestPluginsList_GateRejectsBelowEnterprise` (spawned==0) |
| APITEST_PLUGINS set + Enterprise → host.Load invoked, table rendered | `TestPluginsList_GateAllowsEnterprise` |

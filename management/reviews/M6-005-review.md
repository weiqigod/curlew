...
## Checks
- Prior High finding fix: VERIFIED (`eventsAdapter` has `sensitive *variable.SensitiveSet` and `allowSensitive bool` fields; `RequestEnd` calls `variable.RedactBody` on both bodies; `TestRunCmd_Events_RedactsSensitiveBodyValues` exists in `cmd/curlew/run_test.go`)

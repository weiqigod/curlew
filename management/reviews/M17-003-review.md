...
| Code Organization | PASS | New package `internal/signer/oauth1/` with narrow interface. `init.go` mirrors M17-002 pattern. Single blank import in `cmd/curlew/main.go`. `internal/auth/profile.go` and `internal/signer/signer.go` unchanged (verified via `git diff`). No circular dependencies. |

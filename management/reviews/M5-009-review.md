...
| Code Organization | PASS | `internal/worker` owns its domain; `cmd/curlew/worker.go` is pure flag wiring; no circular dependencies; `defer ticker.Stop()` in `heartbeatLoop`; response body closed in `doWithRetry` |

...
| Code Organization | PASS | `ParallelTAP` lives in `internal/output/tap.go`. `buildParallelMetadata` in `cmd/curlew/main.go` is shared between JSON and TAP paths — no duplication. No circular dependencies introduced. All call sites updated (lines 815, 1488 in `main.go`; test call sites pass `nil` or the new arg correctly). |
...
- Coverage: `internal/output` 92.5% (above 80%), `cmd/curlew` 82.0% (above 80%)

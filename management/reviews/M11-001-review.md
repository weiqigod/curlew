...
| Test Quality | PASS | `TestRun_JUnitFormat_FreeTier` added with explicit `CURLEW_TIER=free` pin, real httptest.Server, XML unmarshaling assertion, and 1-suite×1-testcase structural check; all four gating tests removed; events test migrated to `--parallel 2` with identical assertions; tier elevations cleaned from eight existing JUnit tests; `run_junit_happy` stream discipline cell cleaned of redundant `CURLEW_TIER=professional` env; `env.txt` comment updated (iteration-2 fix confirmed). |
...
| `cmd/curlew/main.go` no longer calls `auth.CheckFeature(reg, "junit_xml", ...)` | PASS | Only a historical comment at line 1018 mentions `junit_xml`; both gate blocks deleted |
| `TestRun_JUnitFormat_FreeTier` drives exit 0 + valid JUnit XML on free tier | PASS | Test exists at `cmd/curlew/main_test.go` line 6283; passes with `CURLEW_TIER=free` explicitly set |
...
- Coverage: 81.6% (via `go test -cover ./cmd/curlew/...`)
